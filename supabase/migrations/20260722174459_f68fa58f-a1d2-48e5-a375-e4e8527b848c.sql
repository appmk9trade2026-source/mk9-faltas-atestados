...
BEGIN
  ...
  -- URL pública oficial do CRM MK9 (fallback quando o chamador não passa link)
...
-- Atualizar mensagens já enfileiradas (não enviadas) para o domínio oficial
UPDATE public.whatsapp_outbox
   SET payload = jsonb_set(
         payload,
         '{link_sistema}',
         to_jsonb('https://mk9-faltas-atestados.lovable.app'::text)
       )
 WHERE evento_tipo = 'USUARIO_CRIADO'
   AND status IN ('PENDENTE','FALHOU_TEMPORARIO')
   AND payload->>'link_sistema' = 'https://mk9-staff-hub.lovable.app';
