-- Atualizar dashboard_metrics com KPIs da Central de Processamento
    ...
    jsonb_build_object(
      ...
      'processados_hoje', (
          ...
          WHERE status_processamento = 'PROCESSADO'
            AND processado_em::date = CURRENT_DATE
            AND (_empresa_id IS NULL OR empresa_id = _empresa_id)
            AND (_projeto_id IS NULL OR projeto_id = _projeto_id)
      ),
      'tempo_medio_processamento_h', (
          SELECT COALESCE(AVG(EXTRACT(EPOCH FROM (processado_em - processamento_iniciado_em))/3600.0), 0)
          FROM public.ausencias 
          WHERE status_processamento = 'PROCESSADO'
            AND processado_em::date BETWEEN _inicio AND _fim
