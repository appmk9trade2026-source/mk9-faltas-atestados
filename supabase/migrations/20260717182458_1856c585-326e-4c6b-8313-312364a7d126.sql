...
BEGIN
  ...
  ) INTO v_kpis FROM filtered;

  -- Colaboradores ativos (não filtrado por período)
  ...
  -- Séries e agregações do período atual (reusando o mesmo filtro)
  WITH filtered AS (
    SELECT a.*, c.nome_completo AS colab_nome, c.supervisor_nome AS sup_nome,
  ...
  INTO v_por_dia, v_por_empresa, v_por_projeto, v_por_tipo, v_por_status,
       v_top_sup, v_top_col, v_tempo_diario, v_heatmap, v_ultimos
  FROM filtered LIMIT 1;

  v_top_emp := v_por_empresa;
  v_top_proj := v_por_projeto;
  ...
  v_result := jsonb_build_object(
    ...
    'top_empresas', COALESCE(v_top_emp,'[]'::jsonb),
    'top_projetos', COALESCE(v_top_proj,'[]'::jsonb)
  );
...
GRANT EXECUTE ON FUNCTION public.dashboard_metrics(date,date,uuid,uuid,text,tipo_ausencia,status_ausencia) TO authenticated;
