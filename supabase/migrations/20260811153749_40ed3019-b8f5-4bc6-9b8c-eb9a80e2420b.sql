-- CRM MK9 — CORREÇÃO CIRÚRGICA: CONSISTÊNCIA GLOBAL AMBEV (FASE 4)
-- Foco: Aplicar regra de impacto zero para faltas justificadas por ocorrência de ponto AMBEV.

-- 1. Atualizar public.dashboard_metrics
...
-- 2. Atualizar public.bi_absenteismo_diario
CREATE OR REPLACE VIEW public.bi_absenteismo_diario AS
 SELECT COALESCE(a.data_inicio, a.created_at::date) AS data_referencia,
  ...
  WHERE a.status_documental = 'ATIVO' -- Somente registros ativos
    AND COALESCE(a.data_inicio, a.created_at::date) IS NOT NULL 
    AND (a.status <> ALL (ARRAY['CANCELADO'::status_ausencia, 'SUBSTITUIDA'::status_ausencia]))
    -- REGRA CANÔNICA FASE 4: Faltas justificadas AMBEV têm impacto operacional zero
    AND NOT (a.tipo = 'FALTA' AND COALESCE(a.status_justificativa, '') = 'JUSTIFICADA_OCORRENCIA_PONTO')
...
GRANT SELECT ON public.bi_absenteismo_diario TO service_role;