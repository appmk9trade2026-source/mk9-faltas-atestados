...
BEGIN
  ...
  BEGIN
    ...
    SELECT
      ...
      c.empresa_id, c.projeto_id, t.categoria_ausencia_id, a.tipo_ausencia_id,
      COALESCE(a.status::text,'PENDENTE'),
      COUNT(*)::int, COUNT(DISTINCT a.colaborador_id)::int,
      COALESCE(SUM(NULLIF(COALESCE(a.dias, a.quantidade_dias_calculada),0)),0)::numeric,
      COALESCE(SUM(NULLIF(COALESCE(a.dias, a.quantidade_dias_calculada),0) * 8),0)::numeric,
      ...
      COUNT(*) FILTER (WHERE COALESCE(a.dias, a.quantidade_dias_calculada) IS NULL OR COALESCE(a.dias, a.quantidade_dias_calculada) = 0)::int
