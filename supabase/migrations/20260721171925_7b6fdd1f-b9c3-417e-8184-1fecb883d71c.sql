...
-- 1) Sequência + coluna
CREATE SEQUENCE IF NOT EXISTS public.projeto_codigo_interno_seq;

ALTER TABLE public.projetos
  ADD COLUMN IF NOT EXISTS codigo_interno text;

-- 2) Backfill (apenas linhas sem código interno)
UPDATE public.projetos
   SET codigo_interno = 'PRJ-' || LPAD(nextval('public.projeto_codigo_interno_seq')::text, 6, '0')
 WHERE codigo_interno IS NULL;

-- 3) Índice único + NOT NULL
CREATE UNIQUE INDEX IF NOT EXISTS projetos_codigo_interno_uidx
  ON public.projetos(codigo_interno);

ALTER TABLE public.projetos
  ALTER COLUMN codigo_interno SET NOT NULL;

-- 4) Trigger: gera código interno automaticamente na criação
CREATE OR REPLACE FUNCTION public.projetos_set_codigo_interno_bi()
RETURNS TRIGGER
LANGUAGE plpgsql
...
BEGIN
  IF NEW.codigo_interno IS NULL OR btrim(NEW.codigo_interno) = '' THEN
    NEW.codigo_interno := 'PRJ-' || LPAD(nextval('public.projeto_codigo_interno_seq')::text, 6, '0');
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS projetos_set_codigo_interno_bi_trg ON public.projetos;
CREATE TRIGGER projetos_set_codigo_interno_bi_trg
  BEFORE INSERT ON public.projetos
  FOR EACH ROW
  EXECUTE FUNCTION public.projetos_set_codigo_interno_bi();

-- 5) Trigger: código interno é imutável após criado
CREATE OR REPLACE FUNCTION public.projetos_codigo_interno_immutable_bu()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF OLD.codigo_interno IS NOT NULL
     AND NEW.codigo_interno IS DISTINCT FROM OLD.codigo_interno THEN
    RAISE EXCEPTION 'PROJECT_CODE_IMMUTABLE: código interno do projeto não pode ser alterado'
      USING ERRCODE = '42501';
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS projetos_codigo_interno_immutable_bu_trg ON public.projetos;
CREATE TRIGGER projetos_codigo_interno_immutable_bu_trg
  BEFORE UPDATE ON public.projetos
  FOR EACH ROW
  EXECUTE FUNCTION public.projetos_codigo_interno_immutable_bu();

-- 6) Alinha a sequência (segurança extra caso rodada duas vezes)
SELECT setval(
  'public.projeto_codigo_interno_seq',
  GREATEST(
    (SELECT COALESCE(MAX(NULLIF(regexp_replace(codigo_interno, '^PRJ-', ''), '')::bigint), 0)
       FROM public.projetos
      WHERE codigo_interno ~ '^PRJ-\d+$'),
    1
  ),
  true
);

-- 7) RPC atualizada: chave lógica Empresa + Projeto (sem coluna Código na planilha).
...
BEGIN
  ...
  -- Chave lógica: Empresa + Projeto (normalizado: trim, collapse espaços, lower)
  ...
  -- Duplicidade no arquivo: mesma Empresa + mesmo Projeto
  INSERT INTO _imp_errors
  ...
  -- Plano: match por empresa_id + nome_norm (chave lógica).
  ...
  FOR r IN SELECT * FROM _imp_plan ORDER BY row_number LOOP
    IF r.projeto_id IS NULL THEN
      -- Código interno é gerado automaticamente pelo trigger BEFORE INSERT.
      IF r.data_cadastro IS NOT NULL THEN
        INSERT INTO public.projetos (empresa_id, nome, descricao, ativo, created_at)
        VALUES (r.empresa_id, r.nome, r.descricao, r.want_ativo, r.data_cadastro)
        RETURNING id INTO v_pid;
      ELSE
        INSERT INTO public.projetos (empresa_id, nome, descricao, ativo)
        VALUES (r.empresa_id, r.nome, r.descricao, r.want_ativo)
        RETURNING id INTO v_pid;
      END IF;
      v_created := v_created + 1;
      v_after := jsonb_build_object(
        'nome', r.nome, 'ativo', r.want_ativo,
        'descricao', r.descricao, 'created_at', r.data_cadastro
      );
      PERFORM public.log_audit_event(
    ...
    ELSE
      ...
      -- NUNCA altera nome, empresa_id, codigo_interno, codigo_projeto, codigo_protocolo, created_at
      UPDATE public.projetos
...
'Importação atômica de projetos (5 colunas: Projeto, Empresa, Descrição, Status, Data cadastro). Chave lógica Empresa+Projeto. Código interno gerado automaticamente pelo trigger.';
