  ...
  LOOP
    BEGIN
      ...
      IF nullif(v_row->>'empresa_id','') IS NOT NULL THEN
        ...
        END IF;
      ELSE
        v_empresa_norm := upper(regexp_replace(trim(coalesce(v_row->>'empresa','')), '\s+', ' ', 'g'));
        ...
        WHERE upper(regexp_replace(trim(nome),'\s+',' ','g')) = v_empresa_norm
          AND ativo = true;
        IF v_count = 0 THEN
          v_errors := v_errors + 1;
          v_details := v_details || jsonb_build_object('linha', v_row->>'linha', 'erro',
      ...
      IF nullif(v_row->>'projeto_id','') IS NOT NULL THEN
        ...
        END IF;
      ELSE
        v_projeto_norm := upper(regexp_replace(trim(coalesce(v_row->>'projeto','')), '\s+', ' ', 'g'));
        ...
        WHERE empresa_id = v_empresa_id
          AND upper(regexp_replace(trim(nome),'\s+',' ','g')) = v_projeto_norm
          AND ativo = true;
        IF v_count = 0 THEN
          v_errors := v_errors + 1;
          v_details := v_details || jsonb_build_object('linha', v_row->>'linha', 'erro',
            format('Projeto "%s" não encontrado na empresa "%s".', v_row->>'projeto', v_row->>'empresa'));
          CONTINUE;
        ELSIF v_count > 1 THEN
          v_errors := v_errors + 1;
          v_details := v_details || jsonb_build_object('linha', v_row->>'linha', 'erro',
            format('Existem vários projetos chamados "%s" na empresa "%s".', v_row->>'projeto', v_row->>'empresa'));
          CONTINUE;
        END IF;
      END IF;
