-- CRM MK9 — CORREÇÃO CIRÚRGICA: RESTAURAÇÃO COMPLETA DO CONTRATO JSON DASHBOARD_METRICS
-- Objetivo: Corrigir chaves incompatíveis (empresa->nome, etc), NaN em ativos, e Heatmap agregado por DOW.
-- Preserva: Regra AMBEV (Fase 4), Contrato 'ultimos', e Segurança (GRANTs).
-- ADICIONADO: top_empresas e top_projetos no JSON final para compatibilidade com ResumoTabs.
...
BEGIN
  ...
  SELECT
    (SELECT jsonb_build_object(
      ...
      -- CORREÇÃO colaboradores_ativos (INTEGER, nunca NULL)
    ...
    -- por_empresa (usado em RankList e ResumoTabs)
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.empresa_id, f.empresa_nome AS nome, COUNT(*) AS total FROM filtered f GROUP BY f.empresa_id, f.empresa_nome
    ) t),
    -- por_projeto (usado em RankList e ResumoTabs)
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.projeto_id, f.projeto_nome AS nome, COUNT(*) AS total FROM filtered f GROUP BY f.projeto_id, f.projeto_nome
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
    ...
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.sup_nome AS nome, COUNT(*) AS total FROM filtered f WHERE f.sup_nome IS NOT NULL GROUP BY f.sup_nome LIMIT 10
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.colaborador_id AS id, f.colab_nome AS nome, COUNT(*) AS total FROM filtered f GROUP BY f.colaborador_id, f.colab_nome LIMIT 10
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.dia), '[]'::jsonb) FROM (
    ...
    -- CORREÇÃO Heatmap (agregado por DOW, removendo hour)
    ...
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.registrado_em DESC), '[]'::jsonb) FROM (
      SELECT f.id, f.registrado_em, f.colab_nome, f.empresa_nome, f.projeto_nome, f.tipo, f.status, f.data_inicio, f.data_fim, f.tipo_oficial_nome, f.tipo_oficial_codigo FROM filtered f LIMIT 10
    ) t),
    -- por_categoria
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT cat.id AS categoria_id, cat.codigo, cat.nome AS nome, cat.cor, COUNT(*) AS total 
      FROM filtered f
    ...
    -- por_tipo_oficial
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.tipo_oficial_id AS tipo_id, f.tipo_oficial_codigo AS codigo, f.tipo_oficial_nome AS nome, f.tipo_oficial_cor AS cor, f.categoria_id, COUNT(*) AS total 
      FROM filtered f 
      WHERE f.tipo_oficial_id IS NOT NULL
      GROUP BY f.tipo_oficial_id, f.tipo_oficial_codigo, f.tipo_oficial_nome, f.tipo_oficial_cor, f.categoria_id
    ) t)
  ...
  v_result := jsonb_build_object(
    ...
    -- RESTAURAÇÃO DE CONTRATO PARA ResumoTabs
    'top_empresas', v_por_empresa,
    'top_projetos', v_por_projeto
  );
...
REVOKE EXECUTE ON FUNCTION public.dashboard_metrics(date, date, uuid, uuid, text, tipo_ausencia, status_ausencia, uuid) FROM public;
