...
-- CRM MK9 — CORREÇÃO DA RPC EXCLUIR_AUSENCIA_SEGURA
-- Objetivo: Garantir que a assinatura canônica exista no banco e o cache seja atualizado.

-- 1. Remover assinaturas conflitantes se existirem (para evitar ambiguidade no overload)
DROP FUNCTION IF EXISTS public.excluir_ausencia_segura(uuid, text, text);

-- 2. Criar a função com a assinatura canônica correta
...
BEGIN
  ...
  -- 3. Verificar existência da ausência e se já não está excluída
  ...
  IF v_ausencia.status_documental = 'EXCLUIDO' THEN
    RETURN jsonb_build_object(
      'success', true,
      'ausencia_id', p_ausencia_id,
      'status_documental', 'EXCLUIDO',
      'message', 'Registro já se encontra excluído.'
    );
  END IF;
  ...
  SET 
    ...
    -- Invalida o status de processamento operacional
    status = 'CANCELADO'
  WHERE id = p_ausencia_id;

  -- 5. Auditoria (chamando a RPC de log existente)
