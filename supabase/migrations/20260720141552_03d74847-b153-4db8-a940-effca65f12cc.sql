...
-- Remove a assinatura antiga (sem _categoria_id) para evitar ambiguidade
DROP FUNCTION IF EXISTS public.dashboard_metrics(date, date, uuid, uuid, text, tipo_ausencia, status_ausencia);
...
REVOKE ALL ON FUNCTION public.dashboard_metrics(date, date, uuid, uuid, text, tipo_ausencia, status_ausencia, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.dashboard_metrics(date, date, uuid, uuid, text, tipo_ausencia, status_ausencia, uuid) TO authenticated, service_role;
