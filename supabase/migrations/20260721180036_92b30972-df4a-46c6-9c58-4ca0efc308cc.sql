...
BEGIN
  ...
  SELECT
    ...
    lower(regexp_replace(btrim(COALESCE(elem->>'empresa_nome','')), '\s+', ' ', 'g')) AS empresa_norm,
  ...
  -- Resolve empresa a partir do próprio catálogo (uma linha por nome
  -- normalizado da planilha). Assim `matches` reflete apenas a quantidade
  -- de empresas cadastradas com aquele nome, sem inflar pela quantidade
  -- de linhas da planilha que apontam para a mesma empresa.
  CREATE TEMP TABLE _imp_empresas ON COMMIT DROP AS
  WITH nomes AS (
    SELECT DISTINCT empresa_norm
      FROM _imp_rows
     WHERE empresa_norm <> ''
  ), matches AS (
    SELECT n.empresa_norm, e.id AS empresa_id, e.ativo
      FROM nomes n
      LEFT JOIN public.empresas e
        ON lower(regexp_replace(btrim(e.nome), '\s+', ' ', 'g')) = n.empresa_norm
  )
  SELECT
    empresa_norm,
    count(empresa_id)                                                          AS matches,
    (array_agg(empresa_id ORDER BY empresa_id) FILTER (WHERE empresa_id IS NOT NULL))[1] AS empresa_id,
    bool_or(COALESCE(ativo, false))                                            AS empresa_ativa
  FROM matches
  ...
  SELECT ir.row_number, 'empresa_nome', 'EMPRESA_AMBIGUOUS',
         'Existem ' || e.matches || ' empresas cadastradas como "' || ir.empresa_original ||
         '". Um administrador precisa consolidar os cadastros antes de importar.'
    FROM _imp_rows ir
