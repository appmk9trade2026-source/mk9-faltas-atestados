...
BEGIN
  ...
  WITH base AS (
    SELECT a.id, a.empresa_id, a.projeto_id, a.colaborador_id, a.tipo, a.status, a.data_inicio, a.data_fim,
           ...
           c.supervisor_usuario_id AS sup_id,
           c.supervisor_nome AS sup_nome_snapshot,
  ...
  SELECT
    ...
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT
          COALESCE(MAX(p.nome), MAX(f.sup_nome_snapshot)) AS nome,
          COUNT(*) AS total
      FROM filtered f
    ...
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.colab_nome AS nome, COUNT(*) AS total FROM filtered f GROUP BY f.colab_nome ORDER BY COUNT(*) DESC LIMIT 10
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.dia), '[]'::jsonb) FROM (
    ...
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.registrado_em DESC), '[]'::jsonb) FROM (
      SELECT f.registrado_em, f.colab_nome, f.tipo, f.status FROM filtered f ORDER BY f.registrado_em DESC LIMIT 10
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
