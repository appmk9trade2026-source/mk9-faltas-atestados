ALTER FUNCTION public.retificar_ausencia(
  p_ausencia_id uuid, 
  ...
  p_arquivo jsonb, 
  p_observacao text
) RENAME TO retificar_ausencia_old_v2;
...
CREATE OR REPLACE FUNCTION public.retificar_ausencia(
  ...
  p_updated_at_check timestamptz DEFAULT NULL::timestamptz,
...
BEGIN
  ...
  ELSE
    -- Etapa 14: Preservar path existente se não houver novo upload
  ...
  -- Etapa 11, 12 e 13: Lógica de Qualidade
  -- Se p_e_erro_supervisor não for informado, derivamos da categoria do motivo
  ...
  -- Etapa 10 — Auditoria Imutável (snapshoting)
  ...
  -- Etapa 8 — Transação Server-Side (Update atômico)
  UPDATE public.ausencias SET
    ...
    -- Etapa 11 e 12: Integrar com Qualidade (preservar erro se já existia ou marcar novo)
  ...
  PERFORM public.log_audit_event(
    ...
    jsonb_build_object('tipo_id', v_a.tipo_ausencia_id, 'tipo', v_a.tipo_ausencia_nome,
                       'periodo_id', v_a.opcao_periodo_id, 'data_inicio', v_a.data_inicio, 'data_fim', v_a.data_fim, 'updated_at', v_a.updated_at),
    jsonb_build_object('tipo_id', v_tipo.id, 'tipo', v_tipo.nome,
                       'periodo_id', v_op.id, 'data_inicio', p_data_inicio, 'data_fim', v_data_fim,
                       'protocolo', v_a.protocolo, 'correlation_id', v_corr, 'papel', v_papel, 'erro_supervisor', v_e_erro_real),
