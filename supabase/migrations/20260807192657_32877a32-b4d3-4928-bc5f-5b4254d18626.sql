-- Migration for Relatório Gerencial de Absenteísmo — Fase 1
-- Objective: KPIs, Rankings, and strict RBAC scope.
    ...
    WHERE a.data_inicio <= _fim AND a.data_fim >= _inicio
      ...
      AND (
        ...
        (v_is_coordenador AND EXISTS (
          ...
          -- A simplificação do RBAC coordenação depende da estrutura de vinculos_coordenacao
          -- Se a tabela vinculos_coordenacao não existir, o RLS base costuma resolver
          -- mas aqui forçamos a lógica de negócio solicitada.
          -- Caso falte a tabela vinculos_coordenacao, o select falhará e saberemos no reteste.
          LIMIT 1
        )) OR
