...
BEGIN
  ...
  SELECT row_number, 'data_cadastro', 'DATE_INVALID',
         'Data cadastro inválida (use DD/MM/YYYY ou YYYY-MM-DD)'
    FROM _imp_rows
   WHERE data_cadastro_raw IS NOT NULL
     AND NOT (data_cadastro_raw ~ '^\d{4}-\d{2}-\d{2}$'
           OR data_cadastro_raw ~ '^\d{2}/\d{2}/\d{4}$');
  ...
  SELECT r1.row_number, 'codigo_projeto', 'DUP_IN_FILE',
   ...
   WHERE r1.empresa_norm <> '' AND r1.codigo_norm <> ''
   GROUP BY r1.row_number;

  ---------------------------------------------------------------------------
  -- 4) Localiza empresas por nome + valida escopo/ambiguidade
  ---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------
  -- 6) Plano de execução (com projeto existente pré-carregado)
  ---------------------------------------------------------------------------
  ...
  SELECT
    ...
    CASE
      WHEN r.data_cadastro_raw ~ '^\d{4}-\d{2}-\d{2}$' THEN r.data_cadastro_raw::timestamptz
      WHEN r.data_cadastro_raw ~ '^\d{2}/\d{2}/\d{4}$' THEN to_date(r.data_cadastro_raw,'DD/MM/YYYY')::timestamptz
      ELSE NULL
  ...
  ---------------------------------------------------------------------------
  -- 7) Permissões calculadas a partir do plano real
  ---------------------------------------------------------------------------
  SELECT
  ...
  FOR r IN SELECT * FROM _imp_plan ORDER BY row_number LOOP
    IF r.projeto_id IS NULL THEN
      -- INSERT novo projeto (data_cadastro → created_at quando informada)
      IF r.data_cadastro IS NOT NULL THEN
        INSERT INTO public.projetos (
          empresa_id, nome, descricao, codigo_protocolo, ativo, created_at
        ) VALUES (
    ...
    ELSE
      ...
      v_before := jsonb_build_object(
        'ativo', v_ativo_atual, 'nome', v_nome_atual, 'descricao', v_desc_atual
      );
      v_after  := jsonb_build_object(
        'ativo', r.want_ativo, 'nome', r.nome, 'descricao', r.descricao
      );

      -- NUNCA altera empresa_id, codigo_protocolo, created_at
...
COMMENT ON FUNCTION public.import_projetos_atomic(jsonb, uuid) IS
  'Importação atômica de projetos (modelo 6 colunas): Projeto, Empresa, Código, Descrição, Status, Data cadastro. Empresa localizada por nome (case-insensitive), com detecção de ambiguidade. Atualiza apenas nome/descrição/status; código, empresa e created_at nunca são alterados; projetos nunca são excluídos.';
