...
-- CRM MK9 — CORREÇÃO CIRÚRGICA: AMBIGUIDADE DE COLUNA NO DASHBOARD_METRICS
-- Objetivo: Resolver o erro "column status does not exist" garantindo aliases explícitos (f.status).
-- Preserva: Contrato JSON (ultimos), Regra AMBEV (Fase 4), e Segurança (GRANTs).
...
BEGIN
  ...
  SELECT
    ...
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.empresa_nome AS empresa, COUNT(*) AS total FROM filtered f GROUP BY f.empresa_nome
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.projeto_nome AS projeto, COUNT(*) AS total FROM filtered f GROUP BY f.projeto_nome
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.tipo AS tipo, COUNT(*) AS total FROM filtered f GROUP BY f.tipo
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.status AS status, COUNT(*) AS total FROM filtered f GROUP BY f.status
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.sup_nome AS supervisor, COUNT(*) AS total FROM filtered f WHERE f.sup_nome IS NOT NULL GROUP BY f.sup_nome LIMIT 10
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.colab_nome AS colaborador, COUNT(*) AS total FROM filtered f GROUP BY f.colab_nome LIMIT 10
    ) t),
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.dia), '[]'::jsonb) FROM (
    ...
    (SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::jsonb) FROM (
      SELECT EXTRACT(DOW FROM f.data_inicio) AS dow,
             EXTRACT(HOUR FROM f.registrado_em) AS hour,
             COUNT(*) AS value
      FROM filtered f GROUP BY dow, hour
    ) t),
    ...
    (SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.total DESC), '[]'::jsonb) FROM (
      SELECT f.tipo_oficial_nome AS tipo, f.tipo_oficial_cor AS cor, COUNT(*) AS total FROM filtered f GROUP BY f.tipo_oficial_nome, f.tipo_oficial_cor
    ) t)
