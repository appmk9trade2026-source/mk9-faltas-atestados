...
-- 2. Garantir permissões para o Data API (authenticated)
-- No Supabase Lovable Cloud, os papéis 'rh', 'compliance' são enums lógicos, não papéis de banco.
-- O controle é feito via RLS ou nas funções SECURITY DEFINER.
...
ON public.ausencias TO authenticated;
...
BEGIN
    -- Validação de Papel (RH, Compliance ou Admin) via has_role (que consulta a tabela user_roles)
    ...
    -- Se já tem responsável, só o admin ou o próprio responsável pode continuar/mudar
...
AS $$
    SELECT json_build_object(
        'backlog', (SELECT count(*)::int FROM public.ausencias WHERE status_processamento = 'AGUARDANDO'),
        'em_processamento', (SELECT count(*)::int FROM public.ausencias WHERE status_processamento = 'EM_PROCESSAMENTO'),
        'processados_hoje', (SELECT count(*)::int FROM public.ausencias WHERE status_processamento = 'PROCESSADO' AND processamento_concluido_em::date = CURRENT_DATE),
        'fora_sla', (SELECT count(*)::int FROM public.ausencias WHERE status_processamento = 'AGUARDANDO' AND (CURRENT_DATE - registrado_em::date) >= 4)
    );
$$;
