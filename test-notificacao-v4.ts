...
async function test() {
  ...
  const { data: aus, error: insErr } = await supabaseAdmin
    ...
    .insert({
      ...
      acidente_trabalho_trajeto: "NAO", // Campo obrigatório pelo trigger
  ...
  await enfileirarNotificacoesAusencia({
    ...
    correlationId: "test-v4-" + Date.now(),
    userId: prof.id
  });
