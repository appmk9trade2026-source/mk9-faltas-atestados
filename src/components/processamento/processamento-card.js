...
import { Play, Check, Eye, Clock, User, Building2, Briefcase, Zap, ArrowRight } from "lucide-react";
...
export function ProcessamentoCard({ data, onIniciar, onConcluir, onReatribuir, onVerDetalhes, isProcessing, currentUserId, isNextInLine }) {
    ...
    return (<Card className={cn("group transition-all hover:shadow-lg border-l-4 relative overflow-hidden", data.prioridade === "CRITICO" ? "border-l-red-500" :
            data.prioridade === "ATENCAO" ? "border-l-amber-500" : "border-l-emerald-500", isNextInLine && "ring-2 ring-primary ring-offset-2")}>
      {isNextInLine && (<div className="absolute top-0 right-0">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between gap-2 mt-1">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {data.cid && (<Badge variant="outline" className="text-[9px] border-primary/30 text-primary px-1.5 h-4">
                  CID: {data.cid}
                </Badge>)}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between border-t pt-3 mt-1">
            <div className="flex flex-col gap-0.5 min-w-0">
              ...
              {data.responsavel_processamento_nome && (<div className="flex items-center gap-1 text-[10px] font-semibold text-blue-600 dark:text-blue-400 truncate">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg" onClick={() => onVerDetalhes(data)} title="Ver Detalhes">
                <Eye className="h-4 w-4"/>
              </Button>

              {isAwaiting && (<Button size="sm" className="h-8 gap-1.5 bg-blue-600 hover:bg-blue-700 text-[11px] font-bold shadow-sm" onClick={() => onIniciar(data.id)} disabled={isProcessing}>
                  <Play className="h-3 w-3 fill-current"/>
                  Assumir
                </Button>)}

              {isInProgress && (<div className="flex gap-1.5 items-center">
                  <Button size="sm" className={cn("h-8 gap-1.5 text-[11px] font-bold shadow-sm", isOwner ? "bg-emerald-600 hover:bg-emerald-700" : "bg-amber-500 hover:bg-amber-600")} onClick={() => {
                if (isOwner)
                    onConcluir(data.id);
                else if (onReatribuir && window.confirm("Assumir este processamento?")) {
                    onReatribuir(data.id, data.responsavel_processamento_id);
                }
            }} disabled={isProcessing} title={!isOwner ? `Em processamento por ${data.responsavel_processamento_nome}. Clique para assumir.` : "Concluir operação"}>
              ...
              {isProcessed && (<Badge variant="outline" className="bg-slate-50 text-slate-600 border-slate-200">
                  PROCESSADO
                </Badge>)}
            </div>
          </div>
