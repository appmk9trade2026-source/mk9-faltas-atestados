import { History, User, FileText, Activity, AlertCircle, CheckCircle2, Shield, Clock, ClipboardList, MessageSquare, FileCheck, Zap, Info, Calendar, MoreVertical, ExternalLink, ArrowRight } from "lucide-react";
...
export function Painel360({ data, onIniciar, onConcluir, onReatribuir, isProcessing, currentUserId }) {
    ...
    const currentStepIndex = data.status_processamento === "PROCESSADO" ? 4
        : data.status_processamento === "EM_PROCESSAMENTO" ? 3
            : data.status_processamento === "AGUARDANDO" ? 2
                : data.lancado_em ? 1 : 0;
    return (<div className="flex flex-col h-full bg-background border-l shadow-2xl animate-in slide-in-from-right duration-300">
        ...
        <div className="flex items-start justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              {data.status_documental === "CONTESTADO" && (<Badge variant="outline" className="text-[10px] font-black uppercase tracking-tighter bg-amber-500 text-white border-amber-600">
                  CONTESTADO
                </Badge>)}
              {data.status_documental === "CANCELADO" && (<Badge variant="outline" className="text-[10px] font-black uppercase tracking-tighter bg-red-500 text-white border-red-600">
                  CANCELADO
                </Badge>)}
              {data.sla_status === "FORA" && (<Badge variant="destructive" className="text-[10px] font-black animate-pulse">
                  FORA DO SLA
                </Badge>)}
        ...
        <div className="p-5 space-y-8">
          <Accordion type="multiple" defaultValue={["dados-colab", "dados-ausencia", "docs", "interno"]}>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="p-3 bg-slate-100 rounded-xl space-y-1 border">
                      ...
                      {(data.horario_inicio || data.horario_fim) && (<div className="mt-1.5 flex items-center gap-1 text-[9px] font-black bg-blue-500/10 text-blue-600 px-1.5 py-0.5 rounded border border-blue-500/20">
                          <Clock className="h-2.5 w-2.5"/>
                          <span>{data.horario_inicio?.slice(0, 5) || "??:??"} - {data.horario_fim?.slice(0, 5) || "??:??"}</span>
                        </div>)}
                      {data.acidente_trabalho && (<Badge variant="destructive" className="text-[8px] h-4 font-black w-full justify-center px-0">ACIDENTE</Badge>)}
                    </div>
                  </div>
            ...
            <AccordionItem value="docs" className="border-none mt-4">
              ...
              <AccordionContent className="pt-2">
                <div className="space-y-3">
                  {data.possui_anexo && data.arquivo_url ? (<div className="p-3 bg-background border rounded-xl flex items-center justify-between group hover:border-primary transition-colors">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-primary hover:text-white transition-all" onClick={async () => {
                try {
                    ...
                    </div>) : (<div className="p-6 bg-muted/20 border rounded-xl border-dashed flex flex-col items-center justify-center text-center space-y-2">
            ...
            return (<div key={step.id} className="relative group">
                          <div className={cn("absolute -left-[23px] top-0 h-[22px] w-[22px] rounded-full border-4 bg-background z-10 flex items-center justify-center transition-all", isActive ? "border-primary bg-primary text-white scale-110 shadow-sm" : "border-slate-50 bg-slate-50 text-slate-300")}>
      ...
      <div className="p-5 border-t bg-slate-50/80 backdrop-blur-sm space-y-3">
        {isAwaiting && (<Button className="w-full bg-blue-600 hover:bg-blue-700 h-12 text-sm font-black shadow-lg hover:shadow-xl transition-all active:scale-95" onClick={() => onIniciar(data.id)} disabled={isProcessing}>
            <Zap className="h-4 w-4 mr-2 fill-current"/> ASSUMIR PROCESSAMENTO
          </Button>)}
        {isInProgress && (<div className="flex gap-2">
             <Button className="flex-1 bg-emerald-600 hover:bg-emerald-700 h-12 text-sm font-black shadow-lg" onClick={() => onConcluir(data.id)} disabled={isProcessing || !isOwner}>
        ...
        {!isOwner && isInProgress && data.responsavel_processamento_id && (<div className="space-y-3">
            ...
            <Button className="w-full bg-amber-500 hover:bg-amber-600 h-12 text-sm font-black shadow-lg" onClick={() => {
        ...
        {data.status_processamento === "PROCESSADO" && (<Button variant="outline" className="w-full h-12 border-emerald-200 text-emerald-600 font-black cursor-default hover:bg-emerald-50">
