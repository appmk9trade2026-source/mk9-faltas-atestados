...
export async function auditExport(payload, formato) {
    try {
        const res = payload.rawResponse;
        const observacoes = [
            `Formato=${formato}`,
        ...
        await supabase.from("audit_logs").insert({
            modulo: "relatorios",
            entidade: payload.id,
            acao: (res?.is_truncated ? "EXPORTACAO_LIMITE_EXCEDIDO" : "EXPORTACAO"),
            observacoes,
            origem: "relatorios",
            sucesso: true,
        });
...
function exportCSV(p) {
    ...
    for (const [k, v] of Object.entries(p.filtrosLabel))
        lines.push(`${k},${v}`);
    lines.push("");
    for (const s of p.sections) {
        lines.push(`# ${s.title}`);
...
export async function exportReport(payload, formato) {
    if (formato === "xlsx")
        exportXLSX(payload);
    else if (formato === "csv")
        exportCSV(payload);
