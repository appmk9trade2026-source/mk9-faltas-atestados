 ...
 */
const ALERT_CONFIG = {
    P0: { cooldown_minutes: 15, persistence_required: false },
    P1: { cooldown_minutes: 60, persistence_required: true, min_occurrences: 3, min_users: 2 },
 ...
 */
function calculateFingerprint(context, error) {
 ...
 */
async function evaluateOperationalAlert(incidentId, context) {
    try {
        ...
        if (config.persistence_required) {
            const p1Config = config;
            const isPersistent = Number(incident.occurrence_count) >= (p1Config.min_occurrences || 0) ||
 ...
 */
async function enqueueOperationalNotification(incident, alertId, status, escalationLevel, traceId) {
...
async function logAlertDecision(incidentId, fingerprint, severity, status, reason, traceId) {
 ...
 */
export async function aggregateIncident(context, error) {
    // 1. Filtros
    if (IGNORED_CATEGORIES.includes(context.category))
        return;
