 ...
 */
export async function enfileirarNotificacoesAusencia({ supabase, ausenciaId, evento, correlationId, userId, }) {
    try {
      ...
      `)
        ...
        const colab = aus.colaborador;
        if (!colab)
            return;
        const projeto = colab.projeto;
        const destinatarios = [];
        ...
        for (const dest of destinatarios) {
            ...
            // 2.1 WhatsApp Outbox
            let templateCodigo = "";
            if (dest.tipo === "COLABORADOR")
                templateCodigo = "AUSENCIA_LANCADA_COLABORADOR_V1";
            else if (dest.tipo === "SUPERVISOR")
                templateCodigo = "AUSENCIA_LANCADA_SUPERVISOR_V1";
            else if (dest.tipo === "RH")
                templateCodigo = "AUSENCIA_LANCADA_RH_V1";
