...
function normalizaComparacao(v) {
    return v.trim().toLowerCase();
}
/**
 * Regras estruturais + contextuais. Não faz chamadas de rede.
 */
export function validarSenhaDefinitiva(pw, ctx = {}) {
    if (pw.length < 8)
        return { ok: false, motivo: "A senha deve ter pelo menos 8 caracteres." };
    if (pw.length > 72)
        return { ok: false, motivo: "A senha deve ter no máximo 72 caracteres." };
 ...
 */
export async function senhaVazada(pw) {
    try {
