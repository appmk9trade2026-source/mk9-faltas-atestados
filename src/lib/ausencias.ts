...
export type TipoAusenciaDetalhe = (typeof TIPO_AUSENCIA_DETALHE)[number];

/** Mapeia o tipo detalhado para o tipo base (enum) usado em regras e templates. */
export function tipoBaseFromDetalhe(detalhe: string): TipoAusencia {
  const d = detalhe.toUpperCase();
  if (d.startsWith("ATESTADO")) return "ATESTADO";
  if (d.startsWith("DECLARAÇÃO") || d.startsWith("DECLARACAO")) return "DECLARACAO";
  if (d.startsWith("FALTA")) return "FALTA";
  if (d.startsWith("SUSPENSÃO") || d.startsWith("SUSPENSAO")) return "SUSPENSAO";
  return "OUTROS";
}
...
export type QuantidadeDiasOption = (typeof QUANTIDADE_DIAS_OPTIONS)[number];

/**
 * Extrai o número de dias do rótulo. Retorna null para períodos não numéricos
 * (ex.: "PERÍODO INTEGRAL", "MEIO PERÍODO").
 */
export function diasFromLabel(label: string): number | null {
