...
import { appRoleSchema, normalizeMatriculaUsuario, rolesExigemMatricula, } from "@/lib/app-roles";
/**
 ...
 */
function checarMatriculaObrigatoria(val, ctx) {
 ...
 */
async function gateUsuario(ctx, permission, route) {
...
async function audit(supabase, acao, registroId, observacoes, antes, depois) {
    await supabase
        .rpc("log_audit_event", {
        _modulo: "usuarios",
        _acao: acao,
...
export const createUsuario = createServerFn({ method: "POST" })
    ...
    if (data.projeto_ids.length) {
        ...
        if (projs.error)
            throw new Error(projs.error.message);
        const check = validarProjetosPertencemAEmpresas(projs.data ?? [], data.projeto_ids, data.empresa_ids);
        if (!check.ok) {
            await audit(context.supabase, "USUARIO_PROJETO_EMPRESA_INCONSISTENTE", null, `Projetos sem empresa vinculada: ${check.invalidos.join(", ")}`);
    ...
    let userId;
    {
        ...
        if (cr.error || !cr.data.user)
            throw new Error(cr.error?.message ?? "Falha ao criar usuário.");
    ...
    const rollback = async (motivo) => {
        ...
        await audit(context.supabase, "USUARIO_CRIACAO_REVERTIDA", userId, `Criação revertida: ${motivo}`);
    };
    try {
        ...
        if (data.roles.length) {
            const rows = data.roles.map((role) => ({ user_id: userId, role: role }));
    ...
    let whatsapp_outbox_id = null;
    let whatsapp_motivo = null;
    if (data.enviar_whatsapp) {
        try {
            const link = getAppPublicUrl();
            const { data: matData, error: matErr } = await supabaseAdmin.rpc("materializar_whatsapp_usuario_boas_vindas", {
                p_user_id: userId,
                p_link_sistema: link,
            ...
            if (matErr) {
                ...
                if (res.ok) {
                    await audit(context.supabase, "ENVIO_COMUNICACAO", userId, `WhatsApp de boas-vindas enfileirado (outbox ${res.outbox_id})`, null, { canal: "whatsapp", template: "USUARIO_CRIADO_V1", outbox_id: res.outbox_id, possui_senha_temporaria: true });
                }
                else {
                    await audit(context.supabase, "ENVIO_COMUNICACAO", userId, `WhatsApp de boas-vindas não enfileirado: ${res.motivo}`, null, { canal: "whatsapp", template: "USUARIO_CRIADO_V1", motivo: res.motivo });
                }
            }
        }
        catch (e) {
...
async function syncSet(opts) {
    ...
    if (toRemove.length) {
        const r = await opts.supabaseAdmin
            ...
            .in(opts.keyColumn, toRemove);
        if (r.error)
            throw new Error(r.error.message);
        for (const v of toRemove)
            await opts.onRemove(v);
    }
}
export const updateUsuario = createServerFn({ method: "POST" })
    ...
    const rolesAtuais = (rolesBefore.data ?? []).map((r) => r.role);
    const empresasAtuais = (empBefore.data ?? []).map((r) => r.empresa_id);
    // ---------- Autoalteração: bloquear perda de privilégios críticos ----------
    if (isSelf) {
        const removeuSuperAdmin = rolesAtuais.includes("super_admin") && !data.roles.includes("super_admin");
        const perdeuEmpresas = empresasAtuais.some((id) => !data.empresa_ids.includes(id));
        if (removeuSuperAdmin) {
            await audit(context.supabase, "USUARIO_AUTOALTERACAO_BLOQUEADA", data.id, "Tentativa de remover próprio papel Super Admin");
            throw new Error("Você não pode remover seu próprio papel Super Admin.");
        }
        if (perdeuEmpresas) {
            await audit(context.supabase, "USUARIO_AUTOALTERACAO_BLOQUEADA", data.id, "Tentativa de remover próprios vínculos de empresa");
    ...
    if (data.projeto_ids.length) {
        ...
        const check = validarProjetosPertencemAEmpresas(projs.data ?? [], data.projeto_ids, data.empresa_ids);
        if (!check.ok) {
            await audit(context.supabase, "USUARIO_PROJETO_EMPRESA_INCONSISTENTE", data.id, `Projetos sem empresa vinculada: ${check.invalidos.join(", ")}`);
            throw new Error("Existem projetos selecionados que não pertencem a uma empresa vinculada ao usuário.");
        }
    }
    // ---------- Último Super Admin ----------
    ...
    if (removeuSuperAdmin) {
        ...
        if (count <= 1) {
            await audit(context.supabase, "USUARIO_ULTIMO_SUPER_ADMIN_BLOQUEADO", data.id, "Tentativa de remover papel Super Admin do último administrador ativo");
    ...
    if (alterouEmail) {
        ...
        await audit(context.supabase, "USUARIO_EDITADO", data.id, "E-mail de acesso alterado", { email: emailAtual }, { email: emailNovo });
    }
    ...
    await audit(context.supabase, "USUARIO_EDITADO", data.id, "Dados atualizados", { nome: profBefore.data.nome, cargo: profBefore.data.cargo, telefone: profBefore.data.telefone_whatsapp, avatar_url: profBefore.data.avatar_url }, { nome: data.nome, cargo: data.cargo, telefone: data.telefone, avatar_url: data.avatar_url });
    return { ok: true };
});
// ---------------- ATIVAR / DESATIVAR ----------------
export const toggleUsuarioAtivo = createServerFn({ method: "POST" })
    ...
    if (data.id === context.userId && !data.ativo) {
        await audit(context.supabase, "USUARIO_AUTOALTERACAO_BLOQUEADA", data.id, "Tentativa de autodesativação");
    ...
    if (!data.ativo) {
        ...
        if (isSuper) {
            ...
            const count = Number(countData ?? 0);
            if (count <= 1) {
                await audit(context.supabase, "USUARIO_ULTIMO_SUPER_ADMIN_BLOQUEADO", data.id, "Tentativa de desativar o último Super Admin ativo");
                throw new Error("É necessário manter pelo menos um Super Admin ativo.");
            }
        }
    }
    ...
    await supabaseAdmin.auth.admin
        ...
        .catch(() => { });
    await audit(context.supabase, data.ativo ? "USUARIO_ATIVADO" : "USUARIO_DESATIVADO", data.id, data.ativo ? "Reativado" : "Desativado");
    return { ok: true };
});
// ---------------- RESET DE SENHA ----------------
...
export const definirSenhaTemporariaUsuario = createServerFn({ method: "POST" })
    ...
    if (data.id === context.userId) {
        await audit(context.supabase, "USUARIO_AUTOALTERACAO_BLOQUEADA", data.id, "Tentativa de redefinir a própria senha via painel administrativo");
        throw new Error("Use a área do seu perfil para trocar a própria senha; este fluxo é para outros usuários.");
    }
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    try {
        const adminAny = supabaseAdmin.auth.admin;
    ...
    await audit(context.supabase, "SENHA_TEMPORARIA_REDEFINIDA", data.id, data.motivo?.trim() ? `Motivo: ${data.motivo.trim()}` : "Senha temporária redefinida pelo administrador", null, {
...
export const redefinirSenhaPadraoUsuario = createServerFn({ method: "POST" })
    ...
    await audit(context.supabase, "SENHA_TEMPORARIA_REDEFINIDA", data.id, `Justificativa administrativa: ${data.motivo.trim()}`, null, {
        email_alvo: prof.data.email,
        padrao: true,
        primeiro_acesso_pendente: true,
        sessoes_encerradas: true,
    ...
    if (prof.data.telefone_whatsapp && prof.data.telefone_whatsapp.trim().length > 0) {
        try {
            ...
            if (matErr) {
                ...
                whatsapp_outbox_id = res.outbox_id ?? null;
                whatsapp_motivo = res.ok ? null : (res.motivo ?? "DESCONHECIDO");
                await audit(context.supabase, "ENVIO_COMUNICACAO", data.id, res.ok
                    ? `WhatsApp de boas-vindas enfileirado após redefinição (outbox ${res.outbox_id})`
                    : `WhatsApp de boas-vindas não enfileirado após redefinição: ${res.motivo}`, null, {
                    canal: "whatsapp",
...
export const contarDependenciasUsuario = createServerFn({ method: "POST" })
    ...
    .handler(async ({ data, context }) => {
    await requireSuperAdmin(context);
    const { data: rpcData, error } = await context.supabase.rpc("contar_dependencias_usuario", { p_user_id: data.id });
    if (error)
...
export const excluirUsuarioSeguro = createServerFn({ method: "POST" })
    ...
    if (data.id === context.userId) {
        await audit(context.supabase, "USUARIO_AUTOALTERACAO_BLOQUEADA", data.id, "Tentativa de auto-exclusão pelo painel administrativo");
    ...
    if (alvoIsSuper) {
        ...
        const count = Number(countData ?? 0);
        if (count <= 1) {
            await audit(context.supabase, "USUARIO_ULTIMO_SUPER_ADMIN_BLOQUEADO", data.id, "Tentativa de excluir o último Super Admin ativo");
    ...
    const { data: depData, error: depErr } = await context.supabase.rpc("contar_dependencias_usuario", { p_user_id: data.id });
    if (depErr)
        throw new Error(depErr.message);
    const dep = (depData ?? {});
    const bloqueios = calcularBloqueiosExclusao(dep);
    if (bloqueios.total > 0) {
        await audit(context.supabase, "USUARIO_EXCLUSAO_BLOQUEADA", data.id, `Exclusão bloqueada por dependências operacionais (total=${bloqueios.total})`, null, bloqueios.detalhes);
        throw new Error("USUARIO_COM_HISTORICO: Este usuário possui registros históricos e não pode ser excluído. Use 'Desativar' para remover o acesso preservando o histórico.");
    }
    // 4. Registra a tentativa (auditoria antes da remoção).
    await audit(context.supabase, "USUARIO_EXCLUSAO_TENTATIVA", data.id, `Exclusão física iniciada para ${prof.data.email}`, { email: prof.data.email, nome: prof.data.nome }, null);
    ...
    if (delAuth.error) {
        ...
        const status = delAuth.error.status;
        await audit(context.supabase, "USUARIO_EXCLUSAO_BLOQUEADA", data.id, `Falha ao excluir identidade de autenticação: ${detalhe}${status ? ` (status=${status})` : ""}`);
        // Repasse legível ao operador — inclui o motivo real (FK, permissão, indisponibilidade).
        throw new Error(`Falha ao excluir a identidade de autenticação: ${detalhe}. Nenhum dado foi removido.`);
    }
    ...
    if (delProf.error) {
        ...
        await audit(context.supabase, "USUARIO_EXCLUSAO_BLOQUEADA", data.id, `Auth removido, mas falha ao remover profile: ${delProf.error.message}`);
        throw new Error("Identidade removida, mas houve falha ao remover o perfil. Contate o administrador do sistema.");
    }
    await audit(context.supabase, "USUARIO_EXCLUIDO", data.id, `Usuário ${prof.data.email} excluído fisicamente (sem histórico).`);
    return { ok: true };
});
export const reenviarConviteUsuario = createServerFn({ method: "POST" })
    ...
    if (!prof.data?.email)
        throw new Error("E-mail do usuário não encontrado.");
    if (prof.data.ativo === false)
        throw new Error("Usuário desativado; reative antes de reenviar convite.");
    ...
    const res = (matData ?? {});
    if (!res.ok) {
        await audit(context.supabase, "USUARIO_CONVITE_REENVIADO", data.id, `Reenvio de convite bloqueado: ${res.motivo}`, null, { canal: "whatsapp", motivo: res.motivo });
        throw new Error(`Não foi possível reenviar convite: ${res.motivo}`);
    }
    await audit(context.supabase, "USUARIO_CONVITE_REENVIADO", data.id, `Convite reenviado (WhatsApp outbox ${res.outbox_id}) para ${prof.data.email}`, null, { canal: "whatsapp", outbox_id: res.outbox_id });
    return { ok: true, outbox_id: res.outbox_id };
});
// ---------------- SESSÕES ATIVAS ----------------
export const encerrarSessoesUsuario = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((data) => z.object({ id: z.string().uuid(), manter_atual: z.boolean().default(true) }).parse(data))
    .handler(async ({ data, context }) => {
    ...
    const scope = isSelf && data.manter_atual ? "others" : "global";
    ...
    await supabaseAdmin
        ...
        .eq("user_id", data.id)
        .eq("status", "ATIVA");
    await audit(context.supabase, "USUARIO_SESSOES_ENCERRADAS", data.id, isSelf && data.manter_atual
...
export const reenviarBoasVindasWhatsapp = createServerFn({ method: "POST" })
    ...
    const res = (matData ?? {});
    if (!res.ok) {
        await audit(context.supabase, "ENVIO_COMUNICACAO", data.id, `Reenvio WhatsApp boas-vindas bloqueado: ${res.motivo}`, null, { canal: "whatsapp", template: "USUARIO_CRIADO_V1", motivo: res.motivo });
        throw new Error(`Não foi possível enfileirar: ${res.motivo}`);
    }
    await audit(context.supabase, "ENVIO_COMUNICACAO", data.id, `Reenvio WhatsApp boas-vindas (outbox ${res.outbox_id})`, null, { canal: "whatsapp", template: "USUARIO_CRIADO_V1", outbox_id: res.outbox_id });
    return { ok: true, outbox_id: res.outbox_id };
});
const ATRASADO_APOS_MIN = 5;
function derivarStatus(raw, createdAt) {
    if (!raw)
        return "NAO_ENVIADO";
    if (raw === "PENDENTE") {
        if (createdAt) {
            const ageMs = Date.now() - new Date(createdAt).getTime();
            if (ageMs > ATRASADO_APOS_MIN * 60000)
                return "ATRASADO";
        }
        return "PENDENTE";
    }
...
export const listarStatusBoasVindas = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((data) => z.object({ user_ids: z.array(z.string().uuid()).max(200) }).parse(data))
    .handler(async ({ data, context }) => {
    ...
    for (const r of rows ?? []) {
        const uid = r.destinatario_usuario_id ?? "";
        if (!uid || byUser.has(uid))
            continue;
        const raw = r.status ?? null;
        ...
        byUser.set(uid, {
            ...
            ultimo_erro: r.ultimo_erro_resumido ?? null,
            ultimo_erro_codigo: r.ultimo_erro_codigo ?? null,
            telefone_mascarado: r.telefone_mascarado ?? null,
            provider_message_id: r.provider_message_id ?? null,
            atualizado_em: (r.confirmado_em ?? r.enviado_em ?? r.falhou_em ?? createdAt) ?? null,
            ...
            template_codigo: r.template_codigo ?? null,
        });
    }
...
export const reprocessarConviteWhatsapp = createServerFn({ method: "POST" })
    ...
    if (!last) {
        const link = getAppPublicUrl();
        const { data: matData, error: matErr } = await supabaseAdmin.rpc("materializar_whatsapp_usuario_boas_vindas", { p_user_id: data.id, p_link_sistema: link, p_senha_temporaria: null });
        if (matErr)
            throw new Error(matErr.message);
        const res = (matData ?? {});
        if (!res.ok)
            throw new Error(`Não foi possível enfileirar: ${res.motivo}`);
        await audit(context.supabase, "WHATSAPP_REENFILEIRADO", data.id, `Convite WhatsApp materializado via reprocessamento`, null, { outbox_id: res.outbox_id });
        return { ok: true, acao: "materializado", outbox_id: res.outbox_id };
    }
    ...
    if (status === "FALHOU_DEFINITIVO" || status === "CANCELADO") {
        ...
        await audit(context.supabase, "WHATSAPP_REENFILEIRADO", data.id, `Convite WhatsApp reenfileirado após ${status}`, null, { outbox_id: last.id });
        return { ok: true, acao: "reenfileirado", outbox_id: last.id };
    }
    ...
    await audit(context.supabase, "WHATSAPP_REENFILEIRADO", data.id, `Convite WhatsApp: próxima tentativa antecipada (estava ${status})`, null, { outbox_id: last.id });
    return { ok: true, acao: "antecipado", outbox_id: last.id };
});
export const listarContasPrimeiroAcessoSuspeitas = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((data) => z.object({ corte: z.string().datetime().optional().nullable() }).parse(data ?? {}))
    .handler(async ({ data, context }) => {
    await requireSuperAdmin(context);
    const { data: rows, error } = await context.supabase.rpc("admin_contas_primeiro_acesso_suspeitas", { _corte: data.corte ?? null });
    if (error)
        throw new Error(error.message);
    return (rows ?? []);
});
