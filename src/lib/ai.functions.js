...
export const scoreCompliance = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((d) => z.object({ motivo: z.string().max(2000) }).parse(d))
    .handler(async ({ data }) => {
    ...
    const raw = await callOpenRouter([
        {
            role: "system",
            content: 'Você avalia motivos de ausência de colaboradores para conformidade com a CLT brasileira. Retorne EXCLUSIVAMENTE JSON com o formato: {"score": <inteiro 0-100>, "label": "Baixo"|"Médio"|"Alto", "feedback": "<1 frase curta em pt-BR>"}. Critérios: clareza, objetividade, adequação legal, ausência de dados médicos sensíveis desnecessários, coerência.',
        },
...
export const improveMotivo = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((d) => z.object({ motivo: z.string().trim().min(1).max(2000) }).parse(d))
    .handler(async ({ data }) => {
    const text = await callOpenRouter([
        {
            role: "system",
            content: "Você atua exclusivamente como assistente de redação profissional. Sua função é melhorar a clareza, ortografia, gramática e organização do texto, tornando-o mais profissional.\n\nVocê NÃO pode alterar o objetivo do texto.\n\nSe o texto é uma observação interna, ele deve continuar sendo uma observação interna.\n\nSe o texto é uma justificativa, ele deve continuar sendo uma justificativa.\n\nSe o texto é uma mensagem ao colaborador, ele deve continuar sendo uma mensagem ao colaborador.\n\nNão crie:\n\n- saudações;\n- despedidas;\n- assinaturas;\n- recomendações;\n- orientações;\n- conclusões;\n- solicitações de providências;\n- textos jurídicos;\n- interpretações trabalhistas.\n\nMelhore apenas a escrita. É terminantemente proibido: citar CLT, leis ou normas; afirmar descontos salariais, advertências, suspensões ou punições; criar consequências ou justificativas; e alterar fatos originais (datas, horários, quantidades, nomes, matrículas ou protocolos). Preserve integralmente o significado informado pelo usuário. Retorne apenas o texto final revisado, sem preâmbulos e sem aspas.",
        },
