...
async function assertRole(
  ...
  role: "super_admin" | "compliance" | "rh",
): Promise<boolean> {
...
async function assertAdminAccess(ctx: {
  ...
  userId: string;
}): Promise<{ role: "super_admin" | "compliance" | "rh" }> {
  if (await assertRole(ctx, "super_admin")) return { role: "super_admin" };
  if (await assertRole(ctx, "compliance")) return { role: "compliance" };
  if (await assertRole(ctx, "rh")) return { role: "rh" };
...
export const getWhatsappHealth = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const durations = (execAggRes.data ?? [])
      .map((r) => r.duracao_ms as number | null)
...
export const getWhatsappProviderConfig = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data } = await context.supabase
      .from("whatsapp_provider_config")
      .select(
        "provider, enabled, modo, instance_name, base_url_public_label, timeout_ms, max_tentativas, retry_base_segundos, retry_max_segundos, batch_size, webhook_enabled, updated_at",
      )
      .order("updated_at", { ascending: false })
      .limit(1)
      .maybeSingle();
...
export const requeueDeadLetter = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { outboxId: string }) => {
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!isSuper) throw new Error("Apenas Super Admin pode reenfileirar mensagens.");
...
export const logWhatsappAdminEvent = createServerFn({ method: "POST" })
  ...
  .inputValidator(
    (input: {
      ...
      if (
        !["WHATSAPP_DEAD_LETTER_VISUALIZADA", "WHATSAPP_EXPORT_OUTBOX", "WHATSAPP_EXPORT_EXECUCOES"].includes(
          input?.acao,
        )
      ) {
