...
export type ExportFormato = "PDF" | "XLSX" | "CSV";

export type BIExportPayload = {
  nome: string;
  filtros: Record<string, string>;
  build?: string;
  ultima_atualizacao?: string | null;
  sections: { title: string; rows: Record<string, string | number | null>[] }[];
  // Aba/segmento CSV (para CSV, exporta somente essa seção; se ausente, primeira)
  csvSection?: string;
};
...
export async function auditBIExport(formato: ExportFormato, filtros: Record<string, string>) {
  try {
    await (supabase as unknown as { from: (t: string) => { insert: (o: unknown) => Promise<unknown> } })
      .from("audit_logs")
      .insert({
        modulo: "bi_executivo",
...
function toXLSX(p: BIExportPayload) {
...
function toCSV(p: BIExportPayload) {
  ...
  for (const [k, v] of Object.entries(p.filtros)) lines.push(`${safeCsvCell(k)},${safeCsvCell(v)}`);
  lines.push("");
  if (rows.length === 0) {
    ...
    for (const r of rows) lines.push(headers.map((h) => safeCsvCell(r[h])).join(","));
  }
...
function toPDF(p: BIExportPayload) {
  ...
  doc.text("Filtros aplicados", M, y); y += 14;
  doc.setFont("helvetica", "normal"); doc.setFontSize(9);
  for (const [k, v] of Object.entries(p.filtros)) { ensure(14); doc.text(`• ${k}: ${v}`, M, y); y += 12; }
  y += 6;

  for (const s of p.sections) {
    ensure(30);
    doc.setFont("helvetica", "bold"); doc.setFontSize(11); doc.setTextColor(37, 99, 235);
    doc.text(s.title, M, y); y += 14;
    doc.setTextColor(30); doc.setFont("helvetica", "normal"); doc.setFontSize(9);
    if (!s.rows.length) { doc.text("Dados insuficientes.", M, y); y += 16; continue; }
    const headers = Object.keys(s.rows[0]);
    const cw = (W - M * 2) / headers.length;
    ensure(18);
    doc.setFillColor(240, 244, 250); doc.rect(M, y - 10, W - M * 2, 16, "F");
    doc.setFont("helvetica", "bold");
...
export async function exportBI(payload: BIExportPayload, formato: ExportFormato) {
  if (formato === "PDF") toPDF(payload);
  else if (formato === "XLSX") toXLSX(payload);
  else toCSV(payload);
