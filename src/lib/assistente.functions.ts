...
async function carregarRoles(supabase: import("@supabase/supabase-js").SupabaseClient, userId: string): Promise<AppRole[]> {
...
export const renomearConversaAssistente = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => z.object({ id: z.string().uuid(), titulo: z.string().trim().min(1).max(120) }).parse(d))
  .handler(async ({ context, data }) => {
    const { error } = await context.supabase
      .from("ai_conversations")
  ...
  .handler(async ({ context, data }) => {
    ...
    try {
      ...
      await context.supabase
        ...
        .update({
          content: out.content,
          structured_content: out.structured as never,
          status: "COMPLETED",
