...
import { resolverPeriodo, periodoAnterior, type PeriodoPreset } from "./periodos.server";
...
export type ToolDefinition<TInput = unknown> = {
  ...
  maxRows: number;
  timeoutMs: number;
  sensitivity: "public" | "operational" | "sensitive";
  execute: (ctx: ToolContext, params: TInput) => Promise<unknown>;
};
...
function zToJson(schema: z.ZodObject<z.ZodRawShape>): Record<string, unknown> {
  ...
  for (const [k, v] of Object.entries(schema.shape)) {
    const def = (v as z.ZodTypeAny)._def;
    const typeName = def?.typeName as string | undefined;
    props[k] = { description: (v as z.ZodTypeAny).description ?? "" };
    if (!(typeName === "ZodOptional" || typeName === "ZodDefault")) required.push(k);
  }
...
const obterResumoOperacional: ToolDefinition<z.infer<typeof resumoInput>> = {
  name: "obter_resumo_operacional",
  description:
    "Agrega ausências, alertas e mensagens WhatsApp de um período. Retorna totais e projetos com maior volume.",
  ...
  execute: async ({ supabase }, params) => {
    ...
    const projPorVolume = new Map<string, number>();
...
const consultarAusencias: ToolDefinition<z.infer<typeof ausenciasInput>> = {
  ...
  execute: async ({ supabase }, params) => {
    ...
    let q = supabase
      ...
      .limit(params.limite);
    if (params.empresa_id) q = q.eq("empresa_id", params.empresa_id);
    if (params.projeto_id) q = q.eq("projeto_id", params.projeto_id);
    if (params.status) q = q.eq("status", params.status);
    if (params.tipo) q = q.ilike("tipo", `%${params.tipo}%`);
    if (params.protocolo) q = q.ilike("protocolo", `%${params.protocolo}%`);
...
const consultarAlertas: ToolDefinition<z.infer<typeof alertasInput>> = {
  ...
  execute: async ({ supabase }, params) => {
    ...
    let q = supabase
      ...
      .limit(params.limite);
    if (params.status) q = q.eq("status", params.status);
    if (params.severidade) q = q.eq("severidade", params.severidade);
    if (params.empresa_id) q = q.eq("empresa_id", params.empresa_id);
    if (params.projeto_id) q = q.eq("projeto_id", params.projeto_id);
    const { data, error } = await q;
    if (error) throw new Error(error.message);
...
const consultarWhatsapp: ToolDefinition<z.infer<typeof whatsappInput>> = {
  ...
  execute: async ({ supabase, roles }, params) => {
    ...
    let q = supabase
      ...
      .limit(params.limite);
    if (params.status) q = q.eq("status", params.status);
    if (params.template_codigo) q = q.eq("template_codigo", params.template_codigo);
    const { data, error } = await q;
    if (error) throw new Error(error.message);
    const isSuper = roles.includes("super_admin");
    const rows = (data ?? []).map((r) =>
      isSuper ? r : { ...r, ultimo_erro: r.ultimo_erro ? "[oculto]" : null },
    );
...
const consultarProjetos: ToolDefinition<z.infer<typeof projetosInput>> = {
  ...
  execute: async ({ supabase }, params) => {
    ...
    if (pjRes.error) throw new Error(pjRes.error.message);
...
const consultarColaboradores: ToolDefinition<z.infer<typeof colabInput>> = {
  ...
  execute: async ({ supabase }, params) => {
    let q = supabase
      ...
      .limit(params.limite);
    if (params.apenas_ativos) q = q.eq("ativo", true);
    if (params.empresa_id) q = q.eq("empresa_id", params.empresa_id);
    if (params.projeto_id) q = q.eq("projeto_id", params.projeto_id);
    const { data, error } = await q;
    if (error) throw new Error(error.message);
    ...
    if (params.sem_telefone_valido) rows = rows.filter((r) => !r.tem_telefone_valido);
...
const compararPeriodos: ToolDefinition<z.infer<typeof compararInput>> = {
  ...
  execute: async ({ supabase }, params) => {
    ...
    async function totais(inicio: string, fim: string) {
      const [aus, ale, wa] = await Promise.all([
...
const consultarProtocolos: ToolDefinition<z.infer<typeof protoInput>> = {
  ...
  execute: async ({ supabase }, params) => {
    ...
    let q = supabase
      ...
      .limit(params.limite);
    if (params.protocolo) q = q.ilike("protocolo", `%${params.protocolo}%`);
    if (params.codigo_projeto) q = q.ilike("protocolo", `${params.codigo_projeto.toUpperCase()}-%`);
    const { data, error } = await q;
    if (error) throw new Error(error.message);
    return { periodo: p, total: data?.length ?? 0, registros: data ?? [] };
  },
...
const obterRelatorioExistente: ToolDefinition<z.infer<typeof relatorioInput>> = {
  ...
  execute: async ({ supabase }, params) => {
    const p = resolverPeriodo(params.periodo);
    const rpcName = `rel_${params.relatorio}` as
      | "rel_absenteismo" | "rel_faltas" | "rel_atestados" | "rel_licencas"
      | "rel_comunicacoes" | "rel_afastamentos_inss" | "rel_medidas_administrativas";
    ...
    if (error) return { periodo: p, erro: error.message, registros: [] };
...
const explicarMetrica: ToolDefinition<z.infer<typeof explicarInput>> = {
...
// --------------- Catálogo ---------------
export const TOOL_CATALOG: ReadonlyArray<ToolDefinition> = [
...
export function toolsForRoles(roles: readonly AppRole[]): ToolDefinition[] {
