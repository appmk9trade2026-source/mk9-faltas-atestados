...
import { callChat, AiProviderError, DEFAULT_MODEL, type ChatMessage, type ToolSpec } from "./ai-provider.server";
import { toolsForRoles, findTool, type ToolContext } from "./tools.server";
...
export type OrchestratorResult = {
  ...
  structured: {
    resposta: string;
    fontes: Array<{ tool: string; params: unknown }>;
    kpis?: Array<{ label: string; valor: string | number; delta?: number | null }>;
    destaques?: string[];
    proximos_passos?: string[];
  };
  toolCalls: Array<{ name: string; params: unknown; ok: boolean; error?: string }>;
  inputTokens: number;
  outputTokens: number;
  latencyMs: number;
  model: string;
...
export async function orquestrar(
  ...
  const toolSpecs: ToolSpec[] = availableTools.map((t) => ({
    type: "function",
  ...
  const messages: ChatMessage[] = [
    ...
    ...historico.slice(-MAX_HISTORY).map((m) => ({ role: m.role, content: scrubString(m.content).slice(0, 4000) }) as ChatMessage),
    { role: "user", content: scrubString(pergunta).slice(0, 4000) },
  ];

  const ctx: ToolContext = { supabase, userId, roles };
  const toolCallsLog: OrchestratorResult["toolCalls"] = [];
  let inputTokens = 0;
  let outputTokens = 0;
  let latency = 0;
  ...
  for (let step = 0; step < MAX_TOOL_STEPS; step++) {
    ...
    if (!msg) throw new AiProviderError("unavailable", "Resposta vazia do provedor");
    ...
    messages.push({ role: "assistant", content: msg.content ?? "", ...(toolCalls.length ? { tool_calls: toolCalls as unknown as never } : {}) } as ChatMessage);
    ...
    for (const call of toolCalls) {
      ...
      try {
        ...
        toolCallsLog.push({ name: tool.name, params: null, ok: false, error: `parametros_invalidos: ${(e as Error).message}` });
        messages.push({
          role: "tool",
          tool_call_id: call.id,
          name: tool.name,
      ...
      try {
        ...
        toolCallsLog.push({ name: tool.name, params, ok: false, error: (e as Error).message });
        messages.push({
          role: "tool",
          tool_call_id: call.id,
          name: tool.name,
  ...
  if (!finalText) {
    ...
    const result = await callChat(
      [...messages, { role: "user", content: "Consolide a resposta final agora, sem chamar mais ferramentas." }],
      { toolChoice: "none", temperature: 0.15 },
    );
    inputTokens += result.raw.usage?.prompt_tokens ?? 0;
    outputTokens += result.raw.usage?.completion_tokens ?? 0;
    latency += result.latencyMs;
