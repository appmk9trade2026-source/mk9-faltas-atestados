...
export const WA_PUBLICO = ["COLABORADOR", "RH", "SUPERVISOR"];
export function maskPhoneDisplay(masked) {
    if (!masked)
        return "—";
...
export function avgSeconds(rows) {
    const vals = rows.map((r) => r.seconds).filter((v) => typeof v === "number" && !Number.isNaN(v));
    if (!vals.length)
        return null;
...
export function execRowTone(row) {
    ...
    if (row.status === "OK" && (ft > 0 || fd > 0))
        return { tone: "warn", label: "PARCIAL" };
    if (row.status === "OK")
        return { tone: "success", label: "SUCESSO" };
