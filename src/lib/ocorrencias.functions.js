...
export const listarOcorrencias = createServerFn({ method: "GET" })
    ...
    if (data.projeto_id)
        query = query.eq("projeto_id", data.projeto_id);
...
export const criarOcorrencia = createServerFn({ method: "POST" })
    ...
    if (data.colaborador_manual) {
        ...
        if (colab.projeto_id !== data.projeto_id)
            throw new Error("Colaborador não pertence ao projeto selecionado.");
        if (colab.supervisor_usuario_id !== data.supervisor_usuario_id)
            throw new Error("Colaborador não pertence ao supervisor selecionado.");
    }
...
export const processarOcorrencia = createServerFn({ method: "POST" })
    ...
    if (!canProcess)
        throw new Error("Apenas RH ou Coordenadores podem processar ocorrências.");
    ...
    if (ocorrencia.status !== "PENDENTE")
        throw new Error("Esta ocorrência já foi processada.");
    let vinculadoAusenciaId = null;
    ...
    const { data: updated, error } = await context.supabase
        ...
        .single();
    if (error)
        throw new Error(`Erro ao processar ocorrência: ${error.message}`);
    // 5. Auditoria
    await supabaseAdmin.rpc("log_audit_event", {
        _modulo: "ocorrencias",
...
export const getSupervisoresProjeto = createServerFn({ method: "GET" })
    ...
    if (error)
        throw new Error(`Erro ao buscar supervisores: ${error.message}`);
    return (supervisores || []);
});
