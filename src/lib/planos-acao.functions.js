...
export const criarPlanoAcao = createServerFn({ method: "POST" })
    .validator((data) => planoAcaoSchema.parse(data))
    ...
    if (input.tipo_alvo === "SUPERVISOR" || input.tipo_alvo === "COLABORADOR") {
        ...
        if (supProjErr || !supProj?.some((s) => s.id === input.supervisor_usuario_id)) {
...
export const listarPlanosAcao = createServerFn({ method: "GET" })
    ...
    if (input?.projeto_id)
        query = query.eq("projeto_id", input.projeto_id);
    ...
    const rows = data ?? [];
    const ids = Array.from(new Set(rows.flatMap((r) => [r.responsavel_usuario_id, r.responsavel_coordenacao_id, r.criado_por_usuario_id].filter(Boolean))));
    let nomes = new Map();
    if (ids.length > 0) {
...
export const calcularSituacao = (plano) => {
    if (plano.status === "CANCELADO")
        return "CANCELADO";
    if (plano.status === "CONCLUIDO") {
        if (plano.resultado_alcancado === "SIM")
            return "CONCLUIDO_SUCESSO";
        if (plano.resultado_alcancado === "PARCIAL")
    ...
    if (plano.acompanhamentos && plano.acompanhamentos.length > 0) {
        ...
        if (differenceInDays(hoje, ultimoCheckin) > 7)
        ...
        if (differenceInDays(hoje, criadoEm) > 7)
            return "SEM_ACOMPANHAMENTO";
    }
    return "NO_PRAZO";
};
export const listarAcompanhamentos = createServerFn({ method: "GET" })
    .validator((data) => z.object({ plano_id: uuid }).parse(data))
    .middleware([requireSupabaseAuth])
    .handler(async ({ data: input, context }) => {
    const { supabase } = context;
    const { data, error } = await supabase
    ...
    const ids = Array.from(new Set(data.map((r) => r.criado_por_usuario_id)));
    let nomes = new Map();
    if (ids.length > 0) {
        const { data: profs } = await supabase
            .from("profiles")
...
Check-ins recentes:
${plano.acompanhamentos?.slice(0, 5).map((a) => `- ${new Date(a.created_at).toLocaleDateString()}: [${a.progresso}%] ${a.observacao}`).join('\n') || "Nenhum check-in registrado."}
