...
export const STATUS_PRECEDENCIA: Record<string, number> = {
  PENDENTE: 0,
  PROCESSANDO: 1,
  FALHOU_TEMPORARIO: 2,
  ENVIADO: 3,
...
export function podeEvoluir(atual: string, novo: string): boolean {
 ...
 */
export function mapEvolutionStatus(raw: unknown): WhatsAppWebhookStatus | null {
...
export type ParsedWebhook = {
  ...
  status: WhatsAppWebhookStatus | null;
  codigo: string | null;
 ...
 */
export function parseEvolutionPayload(body: unknown): ParsedWebhook {
  const b: any = body ?? {};
  const data = b.data ?? b.message ?? b;

  const instance: string | null =
    b.instance ?? b.instanceName ?? data?.instance ?? data?.instanceName ?? null;
  ...
  const codigo: string | null = data?.errorCode ?? b.errorCode ?? null;
