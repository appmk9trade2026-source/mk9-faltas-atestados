...
async function assertSuperAdmin(ctx) {
    ...
    if (data !== true)
        throw new Error("Apenas Super Admin pode usar o modo de teste.");
}
...
export const createTestRecipient = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((raw) => RecipientInput.parse(raw))
...
export const toggleTestRecipient = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((raw) => z.object({ id: z.string().uuid(), ativo: z.boolean() }).parse(raw))
    .handler(async ({ context, data }) => {
    await assertSuperAdmin(context);
...
export const previewTemplateTeste = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((raw) => TesteInput.parse(raw))
    .handler(async ({ context, data }) => {
    await assertSuperAdmin(context);
...
export const enfileirarTemplateTeste = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((raw) => TesteInput.extend({ recipient_id: z.string().uuid() }).parse(raw))
    .handler(async ({ context, data }) => {
    await assertSuperAdmin(context);
...
export const getTesteStatus = createServerFn({ method: "GET" })
    .middleware([requireSupabaseAuth])
    .inputValidator((raw) => z.object({ outbox_id: z.string().uuid() }).parse(raw))
    .handler(async ({ context, data }) => {
    await assertSuperAdmin(context);
    const { data: row, error } = await context.supabase
        .from("whatsapp_outbox")
        .select("id, status, telefone_mascarado, template_codigo, template_versao, provider_message_id, enviado_em, confirmado_em, falhou_em, ultimo_erro_resumido, tentativas")
