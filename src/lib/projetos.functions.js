...
async function audit(supabase, acao, registroId, correlationId, antes, depois, observacoes, empresaId, projetoId, sucesso = true) {
    try {
...
async function findProjetoEquivalente(supabase, empresaId, nome, excludeId) {
    const { data, error } = await supabase.rpc("check_projeto_equivalente", {
...
function equivalenteError(existing) {
    const cod = existing.codigo_interno ? ` (${existing.codigo_interno})` : "";
    return new Error(`PROJETO_EQUIVALENTE: já existe um projeto ativo equivalente nesta empresa — "${existing.nome}"${cod}. Consolide os duplicados antes de criar um novo.`);
}
// ==================== CREATE ====================
export const createProjeto = createServerFn({ method: "POST" })
    ...
    .inputValidator((data) => {
    try {
        return projetoBaseSchema.parse(data);
    }
    catch (e) {
    ...
    if (equivalente)
        throw equivalenteError(equivalente);
    ...
    await audit(context.supabase, "PROJETO_CRIADO", row.id, gate.correlationId, null, payload, "criação", data.empresa_id, row.id);
...
export const updateProjeto = createServerFn({ method: "POST" })
    ...
    if (!current)
        throw new Error("RESOURCE_NOT_FOUND: projeto não encontrado");
    const gate = await requirePermission({
        ctx: context,
    ...
    const antigoCodigo = current.codigo_protocolo ?? "";
    ...
    if (codigoMudou) {
        ...
        if ((count ?? 0) > 0) {
            await audit(context.supabase, "PROJETO_CODIGO_ALTERACAO_NEGADA", data.id, gate.correlationId, { codigo_protocolo: antigoCodigo }, { codigo_protocolo: novoCodigo }, `bloqueado: projeto possui ${count} ausência(s)`, current.empresa_id, data.id, false);
    ...
    if (data.ativo) {
        const equivalente = await findProjetoEquivalente(context.supabase, data.empresa_id, data.nome.trim(), data.id);
        if (equivalente)
            throw equivalenteError(equivalente);
    }
    const payload = {
    ...
    if (error)
        throw mapSupabaseError(error.message);
    await audit(context.supabase, "PROJETO_EDITADO", data.id, gate.correlationId, current, payload, "edição", data.empresa_id, data.id);
    if (codigoMudou) {
        await audit(context.supabase, "PROJETO_CODIGO_ALTERADO", data.id, gate.correlationId, { codigo_protocolo: antigoCodigo }, { codigo_protocolo: novoCodigo }, `código alterado sem ausências vinculadas`, data.empresa_id, data.id);
    }
...
export const setProjetoAtivo = createServerFn({ method: "POST" })
    ...
    if (error)
        throw mapSupabaseError(error.message);
    await audit(context.supabase, data.ativo ? "PROJETO_ATIVADO" : "PROJETO_DESATIVADO", data.id, gate.correlationId, { ativo: current.ativo }, { ativo: data.ativo }, `status alterado`, current.empresa_id, data.id);
    return { ok: true, correlation_id: gate.correlationId };
});
export const getProjetosVinculos = createServerFn({ method: "POST" })
    ...
    .inputValidator((data) => {
    try {
        return z.object({ ids: z.array(uuid).min(1).max(500) }).parse(data);
    }
    catch (e) {
        throw invalidPayload(e);
    }
...
export const deleteProjetosSmart = createServerFn({ method: "POST" })
    ...
    const { data: projetos, error: loadErr } = await context.supabase
        ...
        .in("id", data.ids);
    if (loadErr)
        throw mapSupabaseError(loadErr.message);
            ...
            if (vinculos > 0) {
                if (p.ativo) {
                    ...
                    if (error)
                        throw mapSupabaseError(error.message);
                    if (!upd || upd.length === 0) {
                        throw new Error("PERMISSION_DENIED: sem permissão para arquivar este projeto (RLS)");
                    }
                }
                arquivados += 1;
                detalhes.push({ id: p.id, nome: p.nome, acao: "ARQUIVADO", vinculos });
                await audit(context.supabase, "PROJETO_ARQUIVADO_AUTOMATICO", p.id, correlationId, { ativo: p.ativo }, { ativo: false, colaboradores: nColab, ausencias: nAus }, `arquivado: possui ${nColab} colaborador(es) e ${nAus} ausência(s)` +
                    (data.motivo ? ` — motivo: ${data.motivo}` : ""), p.empresa_id, p.id);
            }
            else {
                ...
                if (affected === 0) {
                    throw new Error("PERMISSION_DENIED: exclusão bloqueada por RLS ou registro já removido (0 linhas afetadas)");
                }
                excluidos += 1;
                detalhes.push({ id: p.id, nome: p.nome, acao: "EXCLUIDO", vinculos: 0 });
                await audit(context.supabase, "PROJETO_EXCLUIDO", p.id, correlationId, {
                    nome: p.nome,
                    ativo: p.ativo,
                    descricao: p.descricao,
                    codigo_protocolo: p.codigo_protocolo,
                }, null, `excluído fisicamente (sem vínculos)` +
                    (data.motivo ? ` — motivo: ${data.motivo}` : ""), p.empresa_id, p.id);
            }
        }
        catch (err) {
    ...
    if (projetos.length > 1) {
        await audit(context.supabase, "PROJETOS_EXCLUSAO_LOTE", null, correlationId, { ids: data.ids }, { excluidos, arquivados, erros: erros.length, detalhes }, `lote: ${excluidos} excluído(s), ${arquivados} arquivado(s), ${erros.length} erro(s)` +
            (data.motivo ? ` — motivo: ${data.motivo}` : ""), null, null);
    }
    return {
...
function normalizeEmpresaNome(v) {
    return _normalizeName(stripInvisible(v));
}
function normalizeNomeProjeto(v) {
    return _normalizeName(stripInvisible(v));
}
async function buildPreview(supabase, correlationId, rows) {
    const empresasBucket = new Map();
    const { data: emps, error: empsErr } = await supabase.from("empresas").select("id, nome, ativo");
    if (empsErr)
        throw mapSupabaseError(empsErr.message);
    for (const e of (emps ?? [])) {
    ...
    for (const r of rows) {
        ...
        if (bucket && bucket.length === 1)
            empresaIds.add(bucket[0].id);
    }
    const projetoKey = new Map();
    if (empresaIds.size > 0) {
        const { data: projs, error: projsErr } = await supabase
            ...
            .eq("ativo", true);
        if (projsErr)
            throw mapSupabaseError(projsErr.message);
    ...
    const linhas = rows.map((r) => {
        ...
        const erros = [];
        if (!enNorm)
            erros.push("Empresa obrigatória");
        if (!nome)
            erros.push("Projeto (nome) obrigatório");
        if (nome.length > 120)
            erros.push("Projeto acima de 120 caracteres");
        ...
        if (enNorm) {
            if (!bucket || bucket.length === 0) {
                ...
                emp = bucket[0];
                if (!emp.ativo)
                    erros.push("Empresa está inativa");
            }
        }
    ...
    const contar = (a) => linhas.filter((l) => l.acao === a).length;
...
export const confirmProjetosImport = createServerFn({ method: "POST" })
    ...
    .inputValidator((data) => {
    try {
        return confirmInputSchema.parse(data);
    }
    catch (e) {
        throw invalidPayload(e);
    }
    ...
    if (previewNow.erro === 0 && rowsJson.length === 0) {
        await audit(context.supabase, "PROJETOS_IMPORTACAO_CONCLUIDA", null, correlationId, null, { duration_ms: 0, total_rows: previewNow.total,
            rows_per_second: 0, created: 0, updated: 0,
            activated: 0, deactivated: 0,
            unchanged: previewNow.ja_existente, correlation_id: correlationId }, `nenhuma criação — ${previewNow.ja_existente} projeto(s) já existente(s)`, null, null, true);
        return {
            criadas: 0,
            atualizadas: 0,
            ativadas: 0,
    ...
    if (previewNow.erro > 0) {
        const detalhes = previewNow.linhas
            ...
            .join(" | ");
        await audit(context.supabase, "PROJETOS_IMPORTACAO_FALHOU", null, correlationId, null, { error_code: "INVALID_PAYLOAD", failure_phase: "rpc_validation",
            duration_ms: 0, total_rows: data.rows.length,
            correlation_id: correlationId, rejected: previewNow.erro }, `bloqueada — ${previewNow.erro} linha(s) com erro`, null, null, false);
    ...
    let rpcErrorHint = null;
    let failurePhase = "unknown";
    const startedAt = Date.now();
    try {
        const { data: out, error } = await context.supabase.rpc("import_projetos_atomic", { _rows: rowsJson, _correlation_id: correlationId });
        if (error) {
            rpcErrorMessage = error.message ?? String(error);
            rpcErrorCode = error.code ?? null;
            rpcErrorDetails = error.details ?? null;
            rpcErrorHint = error.hint ?? null;
        }
        else {
            rpcResult = out;
        }
        ...
        rpcErrorCode = err?.code ?? null;
        rpcErrorDetails = err?.details ?? null;
        rpcErrorHint = err?.hint ?? null;
    }
    if (rpcErrorMessage || !rpcResult) {
        let userCode = "IMPORT_FAILED";
        let userMessage = "Não foi possível concluir a importação. Nenhuma alteração foi aplicada.";
        let phase = "rpc_call";
        const rawUpper = (rpcErrorMessage ?? "").toUpperCase();
        if (/^PERMISSION_DENIED/.test(rpcErrorMessage ?? "")) {
            await audit(context.supabase, "PROJETOS_IMPORTACAO_FALHOU", null, correlationId, null, { error_code: "PERMISSION_DENIED", failure_phase: "rpc_validation",
                duration_ms: Date.now() - startedAt, total_rows: data.rows.length,
                correlation_id: correlationId }, "permissão negada", null, null, false);
            throw new Error("PERMISSION_DENIED: permissão negada para importar projetos");
        }
        if (/^INVALID_PAYLOAD/.test(rpcErrorMessage ?? "")) {
            await audit(context.supabase, "PROJETOS_IMPORTACAO_FALHOU", null, correlationId, null, { error_code: "INVALID_PAYLOAD", failure_phase: "rpc_validation",
                duration_ms: Date.now() - startedAt, total_rows: data.rows.length,
                correlation_id: correlationId }, "payload inválido", null, null, false);
        ...
        await audit(context.supabase, "PROJETOS_IMPORTACAO_FALHOU", null, correlationId, null, { error_code: userCode, failure_phase: failurePhase,
            duration_ms: Date.now() - startedAt, total_rows: data.rows.length,
            correlation_id: correlationId,
            rpc_code: rpcErrorCode, rpc_message: rpcErrorMessage,
            rpc_details: rpcErrorDetails, rpc_hint: rpcErrorHint }, `${userCode} — importação abortada. ${diagBits}`.slice(0, 500), null, null, false);
        const fullMsg = diagBits ? `${userMessage} [${diagBits}]`.slice(0, 480) : userMessage;
        const err = new Error(`${userCode}: ${fullMsg}`);
        err.code = userCode;
    ...
    if (!rpcResult.success) {
        await audit(context.supabase, "PROJETOS_IMPORTACAO_FALHOU", null, correlationId, null, { error_code: "IMPORT_VALIDATION", failure_phase: "rpc_validation",
            duration_ms: rpcResult.duration_ms, total_rows: rpcResult.total,
            correlation_id: correlationId, rejected: rpcResult.rejected,
            errors: rpcResult.errors.slice(0, 50) }, `bloqueada — ${rpcResult.rejected} linha(s) com erro`, null, null, false);
    }
    else {
        const rps = rpcResult.duration_ms > 0
            ? Math.round((rpcResult.total * 1000) / rpcResult.duration_ms)
            : rpcResult.total;
        await audit(context.supabase, "PROJETOS_IMPORTACAO_CONCLUIDA", null, correlationId, null, { duration_ms: rpcResult.duration_ms, total_rows: rpcResult.total,
            ...
            unchanged: rpcResult.unchanged, correlation_id: correlationId }, `importação concluída — ${rpcResult.total} linha(s) em ${rpcResult.duration_ms}ms`, null, null, true);
    }
    return {
