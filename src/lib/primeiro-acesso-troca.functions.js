...
export const concluirPrimeiroAcesso = createServerFn({ method: "POST" })
    ...
    if (!veredito.ok)
        throw new Error(veredito.motivo);
    ...
    if (await senhaVazada(data.nova_senha)) {
        throw new Error("Esta senha aparece em vazamentos públicos de dados. Escolha outra senha.");
    }
    ...
    try {
        ...
        if (probe.data?.session) {
            ...
            await supabase
                .rpc("log_audit_event", {
                _modulo: "auth",
                _acao: "PRIMEIRO_ACESSO_CONCLUIDO",
                _entidade: "Sessão",
                _registro_id: userId,
                ...
                .then(() => { }, () => { });
            const err = new Error("A nova senha deve ser diferente da senha temporária.");
        ...
        if (e?.code === "PASSWORD_EQUALS_TEMPORARY")
            throw e;
    ...
    const criadoEm = prof.data.created_at ? new Date(prof.data.created_at) : null;
    const concluidoEm = new Date();
    const tempoAteAtivacaoMs = criadoEm ? concluidoEm.getTime() - criadoEm.getTime() : null;
    const tempoAteAtivacaoHoras = tempoAteAtivacaoMs !== null ? Math.round((tempoAteAtivacaoMs / 3600000) * 10) / 10 : null;
    const partesObs = [
