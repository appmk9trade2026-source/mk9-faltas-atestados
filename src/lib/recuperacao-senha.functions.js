...
export const solicitarRecuperacaoSenha = createServerFn({ method: "POST" })
    ...
    if (gu.error || !gu.data?.user) {
        await log("IDENTIDADE_AUTH_AUSENTE", gu.error?.name ?? "no_auth_user", gu.error?.message ?? "Perfil sem identidade em auth.users");
        return { ok: true };
    }
    ...
    if (send.error) {
        ...
        const resultado = err.status === 429 ? "RATE_LIMIT" : "RECUPERACAO_FALHOU";
        await log(resultado, codigo, err.message);
        return { ok: resultado === "RATE_LIMIT" ? true : false };
    }
