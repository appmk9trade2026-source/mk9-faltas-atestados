 ...
 */
export function hasPermission(set, code) {
    if (!set)
        return false;
    return set.has(code);
}
 ...
 */
export async function logPermissionDenied(code, ctx) {
    try {
...
export function usePermissions() {
    const [set, setSet] = useState(cache?.set ?? null);
    ...
    const has = useCallback((code) => hasPermission(set, code), [set]);
