...
export type CheckStatus = "ok" | "atencao" | "erro" | "skip";

export type CheckResult = {
  id: string;
  categoria: "health" | "rls" | "dados" | "config" | "rpc";
  titulo: string;
  status: CheckStatus;
  descricao: string;
  recomendacao?: string;
...
export async function runHealthChecks(): Promise<CheckResult[]> {
  ...
  // 1. RPC saude_sistema
  const saude = await timed(() => supabase.rpc("saude_sistema" as never));
  out.push({
  ...
  const score = await timed(() =>
    supabase.rpc("calcular_score_colaboradores_lote" as never, { p_limit: 1 } as never),
  );
  out.push({
  ...
  // 3. RPC has_role
  const hr = await timed(() =>
    supabase.rpc("has_role" as never, { _user_id: "00000000-0000-0000-0000-000000000000", _role: "super_admin" } as never),
  );
  out.push({
    id: "rpc.has_role",
    categoria: "rpc",
  ...
  const cfg = await timed(() =>
    supabase.from("absenteismo_config").select("id").limit(1).maybeSingle(),
  );
...
export async function runRlsChecks(): Promise<CheckResult[]> {
  const out: CheckResult[] = [];

  // Tentativa de leitura em tabelas críticas — apenas confirma que não estoura
  const targets: Array<{ id: string; table: string; label: string }> = [
  ...
  for (const t of targets) {
    const r = await timed(() => supabase.from(t.table as never).select("id" as never, { count: "exact", head: true }));
    out.push({
      id: t.id,
      categoria: "rls",
      ...
      descricao: r.error
        ? `Leitura negada (${(r.error as { message?: string }).message ?? "sem detalhe"})`
...
export type DataIssue = {
  ...
  titulo: string;
  quantidade: number;
  gravidade: "info" | "atencao" | "erro";
  descricao: string;
};

export async function runDataValidation(): Promise<DataIssue[]> {
  ...
  try {
    const dup = await supabase.rpc("diagnose_projetos_duplicados" as never);
    const rows = (dup.data as unknown as unknown[] | null) ?? [];
    issues.push({
