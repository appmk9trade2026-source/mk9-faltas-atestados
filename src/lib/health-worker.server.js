 ...
 */
export async function processNotificationOutbox(dryRun = false) {
...
async function processSingleItem(item, config, dryRun) {
    ...
    try {
        ...
        const eligibleRecipients = config.environment === 'SANDBOX'
            ? filtered.filter(r => r.is_test_recipient)
            : filtered;
...
async function finalizeItem(id, status, attempts, error, providerId) {
    const now = new Date().toISOString();
    await supabaseAdmin
        .from("operational_notification_outbox")
        .update({
 ...
 */
async function realProviderSend(item, recipients) {
    ...
    let lastResult = { success: true, id: null };
