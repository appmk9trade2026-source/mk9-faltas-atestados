...
/** Classifica erros HTTP/rede em TEMPORARIO vs DEFINITIVO. */
export type FailureKind = "TEMPORARIA" | "DEFINITIVA";
 ...
 */
export function classifyEvolutionError(
  status: number | null,
  ...
  if (status === 408) return { kind: "TEMPORARIA", codigo: "TIMEOUT", mensagem: msg };
  if (status === 429) return { kind: "TEMPORARIA", codigo: "HTTP_429", mensagem: msg };
  if (status >= 500 && status < 600) {
    return { kind: "TEMPORARIA", codigo: `HTTP_${status}`, mensagem: msg };
  }
  if (status === 401) return { kind: "DEFINITIVA", codigo: "HTTP_401", mensagem: msg };
  if (status === 404) return { kind: "DEFINITIVA", codigo: "HTTP_404_INSTANCE", mensagem: msg };
  if (status === 400) return { kind: "DEFINITIVA", codigo: "HTTP_400_PAYLOAD", mensagem: msg };
  if (status === 422) return { kind: "DEFINITIVA", codigo: "HTTP_422_TELEFONE", mensagem: msg };
...
export function renderTemplate(
  ...
  variaveisPermitidas: string[] | null | undefined,
): string {
  const permitidas = new Set(variaveisPermitidas ?? []);
  return conteudo.replace(/\{\{\s*([a-zA-Z0-9_.]+)\s*\}\}/g, (_, key: string) => {
    if (permitidas.size > 0 && !permitidas.has(key)) return "";
 ...
 */
export function calcBackoffSeconds(
  tentativas: number,
  baseSeg: number,
  maxSeg: number,
  jitter = Math.random(),
...
export function isEvolutionAccepted(status: number): boolean {
