...
import { Search, RefreshCcw, History, TrendingUp, CheckCircle2, AlertTriangle, Zap, User } from "lucide-react";
...
function KpiCard({ title, value, icon: Icon, color }) {
    return (<Card className="border-none shadow-sm bg-card/50">
...
function CentralProcessamentoPage() {
    ...
    const sortedAndFiltered = useMemo(() => {
        let list = (ausenciasQ.data || []).filter(a => a.colaborador_nome.toLowerCase().includes(search.toLowerCase()) ||
            a.colaborador_matricula.includes(search) ||
            a.protocolo?.toLowerCase().includes(search.toLowerCase()));
        return list.sort((a, b) => {
            // 1. Fora do SLA
            if (a.sla_status === "FORA" && b.sla_status !== "FORA")
                return -1;
            if (b.sla_status === "FORA" && a.sla_status !== "FORA")
                return 1;
            // 2. Maior tempo aguardando
            if (b.tempo_aguardando !== a.tempo_aguardando)
                return b.tempo_aguardando - a.tempo_aguardando;
            // 3. Prioridade Alta
            const pOrder = { CRITICO: 3, ATENCAO: 2, NORMAL: 1 };
            if (pOrder[b.prioridade] !== pOrder[a.prioridade])
    ...
    const iniciarMut = useMutation({
        mutationFn: (id) => iniciarFn({ data: { ausencia_id: id } }),
        onSuccess: () => {
    ...
    const concluirMut = useMutation({
        mutationFn: (id) => concluirFn({ data: { ausencia_id: id } }),
        onSuccess: () => {
    ...
    const reatribuirMut = useMutation({
        mutationFn: (payload) => reatribuirFn({ data: { ausencia_id: payload.id, responsavel_anterior_id: payload.responsavel_anterior_id } }),
        onSuccess: () => {
    ...
    return (<AppShell title="Central de Processamento" breadcrumb={["Operações", "Central de Processamento"]}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {ausenciasQ.isLoading ? Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-48 rounded-xl"/>)
            : sortedAndFiltered.length === 0 ? (<div className="col-span-full py-20 flex flex-col items-center justify-center text-center space-y-3 bg-muted/20 rounded-2xl border-2 border-dashed">
            ...
            </div>) : sortedAndFiltered.map((a, i) => (<ProcessamentoCard key={a.id} data={a} currentUserId={user?.id} isNextInLine={i === 0 && a.status_processamento === "AGUARDANDO"} onIniciar={(id) => iniciarMut.mutate(id)} onConcluir={(id) => concluirMut.mutate(id)} onReatribuir={(id, antId) => reatribuirMut.mutate({ id, responsavel_anterior_id: antId })} onVerDetalhes={(d) => { setRegistroSelecionado(d); setDetalhesAbertos(true); }} isProcessing={iniciarMut.isPending || concluirMut.isPending || reatribuirMut.isPending}/>))}
        </div>
      </div>
      ...
      <Sheet open={detalhesAbertos} onOpenChange={setDetalhesAbertos}>
        <SheetContent className="p-0 sm:max-w-md md:max-w-xl w-full border-none">
          {registroSelecionado && (<Painel360 data={registroSelecionado} onIniciar={(id) => { iniciarMut.mutate(id); setDetalhesAbertos(false); }} onConcluir={(id) => { concluirMut.mutate(id); setDetalhesAbertos(false); }} onReatribuir={(id, antId) => { reatribuirMut.mutate({ id, responsavel_anterior_id: antId }); setDetalhesAbertos(false); }} isProcessing={iniciarMut.isPending || concluirMut.isPending || reatribuirMut.isPending} currentUserId={user?.id}/>)}
        </SheetContent>
      </Sheet>
    </AppShell>);
}
