...
import { Plus, Filter, Loader2, Clock, CheckCircle2, AlertTriangle, Building2, User, Users, ChevronRight, Sparkles } from "lucide-react";
...
import { listarPlanosAcao, criarPlanoAcao, planoAcaoSchema, obterPlanoAcao, listarAcompanhamentos, registrarAcompanhamento, concluirPlano, analisarAndamentoIA } from "@/lib/planos-acao.functions";
...
function PlanosAcaoPage() {
    ...
    const handleGenerateAI = async () => {
        ...
        try {
            ...
            if (res.titulo)
                form.setValue("titulo", res.titulo);
            if (res.problema_revisado)
                form.setValue("problema_identificado", res.problema_revisado);
            if (res.meta)
                form.setValue("meta", res.meta);
            if (res.indicador_sucesso)
                form.setValue("indicador_sucesso", res.indicador_sucesso);
            if (res.acao_proposta)
                form.setValue("acao_proposta", res.acao_proposta);
    ...
    const mutation = useMutation({
        mutationFn: (data) => createPlanoFn({ data }),
        onSuccess: () => {
    ...
    const kpis = {
        ...
        atrasados: planos?.filter((p) => p.situacao === "ATRASADO").length || 0,
        atencao: planos?.filter((p) => p.situacao === "ATENCAO").length || 0,
        sem_acompanhamento: planos?.filter((p) => p.situacao === "SEM_ACOMPANHAMENTO").length || 0,
    ...
    return (<AppShell title="Plano de Ação Gerencial">
      <div className="space-y-6">
        ...
        {resumoGerencial && (<Card className="bg-primary/5 border-primary/20 animate-in fade-in slide-in-from-top-2 duration-300 relative">
            <Button variant="ghost" size="icon" className="absolute top-2 right-2 h-6 w-6" onClick={() => setResumoGerencial(null)}>
        ...
        <Card>
          ...
          <CardContent className="p-0">
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (<TableRow>
                  ...
                  </TableRow>) : !planos?.length ? (<TableRow>
                    <TableCell colSpan={8} className="h-24 text-center text-muted-foreground">
                      Nenhum plano de ação encontrado.
                    </TableCell>
                  </TableRow>) : ([...planos].sort((a, b) => {
            const order = {
                "ATRASADO": 1,
                "ATENCAO": 2,
            ...
            return (order[a.situacao] || 9) - (order[b.situacao] || 9);
        }).map((plano) => (<TableRow key={plano.id}>
                      <TableCell className="font-medium">{plano.titulo}</TableCell>
                      <TableCell>
                        <Badge variant="secondary">{plano.tipo_alvo}</Badge>
                      </TableCell>
                      <TableCell>{plano.projeto?.nome || "-"}</TableCell>
                      <TableCell>
                         <div className="text-xs text-muted-foreground">
                            {plano.supervisor?.nome ? `S: ${plano.supervisor.nome}` : ''}
                            {plano.colaborador?.nome_completo ? ` | C: ${plano.colaborador.nome_completo}` : ''}
                         </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-xs">
                          {plano.responsavel_tipo === "USUARIO" ? (<span className="flex items-center gap-1"><User className="h-3 w-3"/> {plano.responsavel_usuario?.nome || "-"}</span>) : (<span className="flex items-center gap-1 font-semibold text-primary"><Users className="h-3 w-3"/> {plano.responsavel_coordenacao?.nome || "Coordenação"}</span>)}
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={plano.situacao === "ATRASADO" ? "bg-red-100 text-red-800" :
          ...
          <Form {...form}>
             <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <FormField control={form.control} name="tipo_alvo" render={({ field }) => (<FormItem>
                 ...
                 <FormField control={form.control} name="projeto_id" render={({ field }) => (<FormItem>
                ...
                form.setValue("colaborador_id", null);
                 ...
                 {(tipoAlvo === "SUPERVISOR" || tipoAlvo === "COLABORADOR") && (<FormField control={form.control} name="supervisor_usuario_id" render={({ field }) => (<FormItem>
                         <FormLabel>Supervisor</FormLabel>
                         <Select onValueChange={(val) => {
                    field.onChange(val);
                    form.setValue("colaborador_id", null);
                }} value={field.value || ""} disabled={isLoadingSupervisores || isUserSupervisor}>
                           <FormControl>
                             <SelectTrigger>
                 ...
                 {tipoAlvo === "COLABORADOR" && (<FormField control={form.control} name="colaborador_id" render={({ field }) => (<FormItem>
                         ...
                         <SelectContent>
                           <div className="p-2">
                             <Input placeholder="Filtrar colaboradores..." value={buscaColab} onChange={(e) => setBuscaColab(e.target.value)} className="h-8 mb-2"/>
                           </div>
                           {isLoadingColaboradores ? (<div className="p-2 text-center text-xs text-muted-foreground">Carregando...</div>) : colaboradores?.length === 0 ? (<div className="p-2 text-center text-xs text-muted-foreground">Nenhum encontrado</div>) : (colaboradores?.map(c => (<SelectItem key={c.id} value={c.id}>{c.nome_completo} ({c.matricula})</SelectItem>)))}
                         </SelectContent>
                       </Select>
                       <FormMessage />
                     </FormItem>)}/>)}
               ...
               <FormField control={form.control} name="titulo" render={({ field }) => (<FormItem>
               ...
               <div className="space-y-4">
                 <FormField control={form.control} name="problema_identificado" render={({ field }) => (<FormItem>
                       <div className="flex items-center justify-between">
                         <FormLabel>Problema Identificado</FormLabel>
                         <Button type="button" variant="ghost" size="sm" className="h-7 text-xs text-primary" onClick={handleGenerateAI} disabled={isGeneratingAI}>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <FormField control={form.control} name="meta" render={({ field }) => (<FormItem>
                   ...
                   <FormField control={form.control} name="acao_proposta" render={({ field }) => (<FormItem>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <FormField control={form.control} name="indicador_sucesso" render={({ field }) => (<FormItem>
                   ...
                   <FormField control={form.control} name="prioridade" render={({ field }) => (<FormItem>
                         ...
                         <Select onValueChange={field.onChange} defaultValue={field.value}>
                           ...
                           <SelectContent>
                             {Object.entries(PRIORIDADE_LABELS).map(([val, label]) => (<SelectItem key={val} value={val}>{label}</SelectItem>))}
                           </SelectContent>
                         </Select>
                         <FormMessage />
                       </FormItem>)}/>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                   ...
                   <FormField control={form.control} name="data_inicio" render={({ field }) => (<FormItem>
                   ...
                   <FormField control={form.control} name="prazo" render={({ field }) => (<FormItem>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField control={form.control} name="responsavel_tipo" render={({ field }) => (<FormItem>
                ...
                form.setValue("responsavel_coordenacao_id", null);
                    ...
                    {form.watch("responsavel_tipo") === "USUARIO" ? (<FormField control={form.control} name="responsavel_usuario_id" render={({ field }) => (<FormItem>
                            ...
                            <FormMessage />
                          </FormItem>)}/>) : (<FormField control={form.control} name="responsavel_coordenacao_id" render={({ field }) => (<FormItem>
                            ...
                            <Select onValueChange={field.onChange} value={field.value || ""}>
                              ...
                              <SelectContent>
                                ...
                                {(!coordenadores || coordenadores.length === 0) && (<SelectItem value="none" disabled>Nenhuma coordenação encontrada</SelectItem>)}
                              </SelectContent>
                            </Select>
                            <FormMessage />
                          </FormItem>)}/>)}
...
function PlanoDetalhe({ planoId, onClose }) {
    const queryClient = useQueryClient();
    ...
    const concluirForm = useForm({
        defaultValues: {
            resultado_alcancado: "SIM",
            parecer_final: "",
        },
    });
    const checkinMutation = useMutation({
        mutationFn: (data) => addCheckinFn({ data: { plano_id: planoId, ...data } }),
        onSuccess: () => {
    ...
    const concluirMutation = useMutation({
        mutationFn: (data) => concluirFn({ data: { id: planoId, ...data } }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["plano-acao", planoId] });
            queryClient.invalidateQueries({ queryKey: ["planos-acao"] });
    ...
    return (<div className="space-y-6 pb-20">
      ...
      <div>
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-1 ml-auto border-l pl-2">
            ...
            {plano.responsavel_tipo === "USUARIO" ? (<span className="flex items-center gap-1"><User className="h-3 w-3"/> {plano.responsavel_usuario?.nome}</span>) : (<span className="flex items-center gap-1"><Users className="h-3 w-3"/> Coordenação: {plano.responsavel_coordenacao?.nome}</span>)}
          </div>
        </div>
      ...
      {plano.status === "CONCLUIDO" && (<div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        {plano.status !== "CONCLUIDO" && plano.status !== "CANCELADO" && (<Button size="sm" onClick={() => {
        ...
        {plano.status !== "CONCLUIDO" && plano.status !== "CANCELADO" && (<Button size="sm" variant="outline" onClick={() => setIsConcluirDialogOpen(true)}>
            Concluir Plano
          </Button>)}
      ...
      {iaAnalysis && (<Card className="bg-primary/5 border-primary/20">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4 border-l-2 border-muted ml-3 pl-6 relative">
          ...
          {checkins?.map((c) => (<div key={c.id} className="relative">
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input type="number" min="0" max="100" value={checkinForm.watch("progresso")} onChange={(e) => checkinForm.setValue("progresso", parseInt(e.target.value))}/>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Observação</label>
              <Textarea placeholder="O que foi feito?" value={checkinForm.watch("observacao")} onChange={(e) => checkinForm.setValue("observacao", e.target.value)}/>
            </div>
          </div>
          <DialogFooter>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Select value={concluirForm.watch("resultado_alcancado")} onValueChange={(v) => concluirForm.setValue("resultado_alcancado", v)}>
            ...
            <div className="space-y-2">
              ...
              <Textarea placeholder="Descreva o resultado final e aprendizados..." value={concluirForm.watch("parecer_final")} onChange={(e) => concluirForm.setValue("parecer_final", e.target.value)}/>
            </div>
          </div>
          <DialogFooter>
